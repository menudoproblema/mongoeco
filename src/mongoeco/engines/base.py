...
class AsyncCrudEngine(Protocol):
    ...
    async def update_with_operation(self, db_name: str, coll_name: str, operation: UpdateOperation, upsert: bool = False, upsert_seed: Document | None = None, *, selector_filter: Filter | None = None, dialect: MongoDialect | None = None, context: ClientSession | None = None) -> UpdateResult[DocumentId]: ...
...
class AsyncIndexAdminEngine(Protocol):
    async def create_index(self, db_name: str, coll_name: str, keys: IndexKeySpec, *, unique: bool = False, name: str | None = None, sparse: bool = False, partial_filter_expression: Filter | None = None, max_time_ms: int | None = None, context: ClientSession | None = None) -> str: ...
