...
class _SQLiteReadRuntimeEngine(Protocol):
    ...
    def _plan_has_array_traversing_paths(
        self, db_name: str, coll_name: str, plan: object
    ) -> bool: ...
    ...
    def _sort_requires_python(
        self, db_name: str, coll_name: str, plan: object, sort: object | None
    ) -> bool: ...
    ...
    def _find_scalar_fast_path_index(
        self, db_name: str, coll_name: str, field: str
    ): ...
    ...
    def _plan_find_semantics_sync(
        self, db_name: str, coll_name: str, semantics: EngineFindSemantics
    ): ...


def _python_text_execution_plan(
    ...
    return SQLiteReadExecutionPlan(
        ...
        execution_lineage=(
            ExecutionLineageStep(
                runtime="python", phase="scan", detail="sqlite collection scan"
            ),
            ExecutionLineageStep(
                runtime="python", phase="text", detail="classic text filter"
            ),
            ExecutionLineageStep(
                runtime="python", phase="filter", detail="semantic core"
            ),
        ),
        physical_plan=(
...
def compile_read_execution_plan(
    ...
    *,
    ...
    return _sqlite_compile_read_execution_plan(
        ...
        plan_requires_python_for_decimal_numeric=engine._plan_requires_python_for_decimal_numeric,
...
def plan_find_semantics_sync(
    ...
    if semantics.text_query is not None:
        ...
        if semantics.sort:
            lineage.append(
                ExecutionLineageStep(
                    runtime="python", phase="sort", detail="semantic core"
                )
            )
    ...
    return _sqlite_plan_find_semantics_sync(
        ...
        find_scalar_fast_path_index=lambda current_db_name, current_coll_name, field: (
            engine._find_scalar_fast_path_index(
                current_db_name,
                current_coll_name,
                field,
            )
        ),
        field_is_top_level_array_in_collection=lambda current_db_name, current_coll_name, field: (
            engine._field_is_top_level_array_in_collection(
                current_db_name,
                current_coll_name,
                field,
            )
        ),
        field_contains_real_numeric_in_collection=lambda current_db_name, current_coll_name, field: (
            engine._field_contains_real_numeric_in_collection(
                current_db_name,
                current_coll_name,
                field,
            )
        ),
        field_contains_non_ascii_text_in_collection=lambda current_db_name, current_coll_name, field: (
            engine._field_contains_non_ascii_text_in_collection(
                current_db_name,
                current_coll_name,
                field,
            )
        ),
        build_equals_sql=lambda current_db_name, current_coll_name, value, index_name, physical_name, limit, null_matches_undefined: (
            engine._build_scalar_indexed_top_level_equals_sql(
                current_db_name,
                current_coll_name,
        ...
        build_range_sql=lambda current_db_name, current_coll_name, value, operator, index_name, physical_name, limit: (
            engine._build_scalar_indexed_top_level_range_sql(
                current_db_name,
                current_coll_name,
                field=semantics.query_plan.field,
                value=value,
        ...
        compile_read_plan=lambda current_semantics, current_hint: (
            engine._compile_read_execution_plan(
                db_name,
                coll_name,
                current_semantics,
                hint=current_hint,
