...
def _migrate_change_outbox_v2(conn: sqlite3.Connection) -> None:
    ...
    if "effect_hash" not in identity_columns:
        conn.execute(
            "ALTER TABLE change_outbox_identities ADD COLUMN effect_hash TEXT",
        )
    conn.execute(
        """
...
def _migrate_change_outbox_v4(conn: sqlite3.Connection) -> None:
    ...
    if "owner_instance" not in consumer_columns:
        conn.execute(
            "ALTER TABLE change_outbox_consumers ADD COLUMN owner_instance TEXT",
        )
...
def reusable_consumer_checkpoint(
    ...
    *,
    ...
    """Read an unchanged, live registration without opening a write transaction.

    Repeated preparation before a data write is not a lease renewal. Updating
    its timestamp through the control connection would invalidate an already
    open WAL read snapshot on the data connection. The registration heartbeat
    owns periodic renewal; explicit changes still use register_consumer.
    """
    row = conn.execute(
        """
        SELECT checkpoint, durable, owner_instance,
               registration_expires_at_epoch > CAST(strftime('%s', 'now') AS REAL)
    ...
    if row is None:
        return None
    checkpoint, registered_durable, registered_owner, alive = row
    if durable and not registered_durable:
        return None
    if not registered_durable and (registered_owner != owner_instance or not alive):
        return None
    if initial_checkpoint is not None and initial_checkpoint > checkpoint:
        return None
    # Do not turn a local registration cache into authority for a checkpoint:
    # another engine may have advanced it or compacted the retained history.
    return consumer_checkpoint(conn, consumer_id)
...
def register_consumer(  # noqa: PLR0913 - persisted registration boundary
    ...
    *,
    ...
    if durable and (owner_instance is not None or ephemeral_ttl_seconds is not None):
    ...
    if (owner_instance is None) is not (ephemeral_ttl_seconds is None):
        message = "ephemeral owner and registration TTL must be provided together"
    ...
    if initial_checkpoint is not None and initial_checkpoint > newest_sequence:
        message = (
    ...
    if row is None:
        checkpoint = (
            newest_sequence if initial_checkpoint is None else initial_checkpoint
        )
...
def compact_change_outbox(
    ...
    *,
    ...
    acknowledged_floor = max_sequence if row is None or row[0] is None else int(row[0])
...
def append_change(  # noqa: PLR0913 - transactional outbox boundary
    ...
    *,
    ...
    if context is None or context.publication is ChangePublicationPolicy.DISABLED:
        return None
    kind = (
        "gap" if context.publication is ChangePublicationPolicy.RECORD_GAP else "event"
    )
    ...
    if existing is not None:
        ...
        if existing[1] is None or existing[2] is None:
            message = "cannot verify replay for a legacy compacted outbox identity"
            raise OperationFailure(message)
        if existing[1] != kind or existing[2] != expected_hash:
            message = "outbox identity was reused with a different change payload"
            raise OperationFailure(message)
...
def read_committed_changes(
    ...
    *,
    ...
    if after_sequence < floor:
        ...
        raise OperationFailure(message)
    upper_bound = _max_sequence(conn) if through_sequence is None else through_sequence
    rows = conn.execute(
        """
    ...
    return tuple(
        CommittedChange(
            ...
            payload=(None if kind == "gap" else deserialize_document(payload)),
        )
