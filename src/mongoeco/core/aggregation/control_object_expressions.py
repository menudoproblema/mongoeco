...
type ExpressionEvaluator = Callable[
    [Document, object, dict[str, Any] | None], Any
]
type ExpressionArgValidator = Callable[
    [str, object, int, int | None], list[Any]
]
...
type PickNInputEvaluator = Callable[
    [str, Document, object, dict[str, Any] | None], tuple[Any, int]
]
...
def _evaluate_field_bound_match_expression(
    ...
    *,
    ...
    if not _is_field_path_expression(
        field_expression
...
def evaluate_control_object_expression(
    ...
    *,
    ...
    if operator == '$and':
        ...
        return all(
            expression_truthy(evaluate_expression(document, item, variables))
            for item in args
        )
    ...
    if operator == '$setField':
        ...
        if (
            ...
            or isinstance(input_value, UndefinedType)
        ):
            return None
    ...
    if operator == '$getField':
        ...
        if (
            ...
            or source is None
            or isinstance(source, UndefinedType)
        ):
            return None
