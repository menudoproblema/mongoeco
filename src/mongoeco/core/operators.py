...
class UpdateEngine:
    ...
    def apply_update(doc: dict[str, Any], update_spec: dict[str, Any]) -> bool:
        ...
        for op, params in update_spec.items():
            ...
            if op == "$set":
                ...
                if UpdateEngine._apply_inc(doc, params):
                    modified = True
            elif op == "$push":
                if UpdateEngine._apply_push(doc, params):
                    modified = True
            elif op == "$addToSet":
                if UpdateEngine._apply_add_to_set(doc, params):
                    modified = True
            elif op == "$pull":
                if UpdateEngine._apply_pull(doc, params):
                    modified = True
            elif op == "$pop":
                if UpdateEngine._apply_pop(doc, params):
                    modified = True
            else:
    ...
    @staticmethod
    def _iter_ordered_update_items(params: dict[str, Any]) -> list[tuple[str, Any]]:
    ...
    def _apply_set(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, value in UpdateEngine._iter_ordered_update_items(params):
    ...
    def _apply_unset(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, _ in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
    ...
    @staticmethod
    def _apply_inc(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, increment in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
    ...
    @staticmethod
    def _apply_push(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, value in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
    ...
    @staticmethod
    def _apply_add_to_set(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, value in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
            ...
            if not found:
                ...
                for candidate in values:
                    if any(QueryEngine._values_equal(existing, candidate) for existing in unique_values):
                        continue
            ...
            for candidate in values:
                if any(QueryEngine._values_equal(existing, candidate) for existing in current):
                    continue
    ...
    @staticmethod
    def _apply_pull(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, value in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
            found, current = get_document_value(doc, path)
                ...
                filtered = [
                    ...
                    if not (
                        (
                            ...
                            else QueryEngine._match_elem_match_candidate(candidate, value)
                        ) if is_predicate else QueryEngine._values_equal(candidate, value)
                    )
                ]
            else:
                filtered = [candidate for candidate in current if not QueryEngine._values_equal(candidate, value)]
            if filtered != current:
    ...
    @staticmethod
    def _apply_pop(doc: dict[str, Any], params: dict[str, Any]) -> bool:
        modified = False
        for path, direction in UpdateEngine._iter_ordered_update_items(params):
            UpdateEngine._assert_mutable_path(path)
