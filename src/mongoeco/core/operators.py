...
if TYPE_CHECKING:
    from mongoeco.types import ArrayFilters, Filter, Update
...
class UpdateEngine:
    ...
    _OPERATOR_HANDLERS: dict[str, UpdateOperatorHandler] = {
        ...
        "$currentDate": lambda doc, instructions, context, variables: (
            apply_current_date(
                doc,
                instructions,
                context=context,
        ...
        "$setOnInsert": lambda doc, instructions, context, variables: (
            apply_set_on_insert(
                doc,
                instructions,
                context=context,
        ...
        "$addToSet": lambda doc, instructions, context, variables: (
            apply_add_to_set(
                doc,
                instructions,
                context=context,
        ...
        "$pullAll": lambda doc, instructions, context, variables: (
            apply_pull_all(
                doc,
                instructions,
                context=context,
    ...
    def _resolve_update_targets(
        ...
        *,
        ...
        compiled_path = (
            path if isinstance(path, CompiledUpdatePath) else compile_update_path(path)
        )
        ...
        concrete_paths = resolve_positional_update_paths(
            ...
            legacy_matcher=(
                ...
                or not any(segment.kind == "positional" for segment in segments)
    ...
    def _build_legacy_positional_matcher(
        ...
        *,
        ...
        def _build(filter_spec: Filter) -> Callable[[Any], bool] | None:
                    ...
                    if and_matchers:
                        matchers.append(
                            lambda candidate, and_matchers=tuple(and_matchers): (
                                all(matcher(candidate) for matcher in and_matchers)
                            ),
                        )
                    continue
                    ...
                    if or_matchers:
                        matchers.append(
                            lambda candidate, or_matchers=tuple(or_matchers): (
                                any(matcher(candidate) for matcher in or_matchers)
                            ),
                        )
                    continue
            ...
            if not matchers:
                return None
            return lambda candidate: all(matcher(candidate) for matcher in matchers)
