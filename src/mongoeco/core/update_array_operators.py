        ...
        if sort_value in (1, -1) and not isinstance(sort_value, bool):
            ...
            for field, direction in sort_value.items():
                if not isinstance(field, str):
                    raise OperationFailure(
                        '$push $sort fields must be strings'
                    )
                if direction not in (1, -1) or isinstance(direction, bool):
                    raise OperationFailure(
                        '$push $sort directions must be 1 or -1'
                    )
    ...
    *,
    ...
    if values:
        if position is None:
            ...
            insertion_index = (
                position if position >= 0 else len(current) + position
            )
...
def get_or_create_array_target(
    ...
    *,
    ...
    if not isinstance(current, list):
        raise OperationFailure(
            f'{operator_name} requires the target field to be an array'
        )
    return current, False


def _pull_candidate_matches(
    candidate: Any,
    condition: Any,
    *,
    context: 'UpdateExecutionContext',
) -> bool:
    if isinstance(condition, (re.Pattern, Regex)):
        compiled_regex = (
            condition.compile() if isinstance(condition, Regex) else condition
        )
        return (
            isinstance(candidate, str)
            and compiled_regex.search(candidate) is not None
        )
    if isinstance(condition, dict):
        operator_keys = query_operator_keys(condition)
        if operator_keys and (
            len(operator_keys) != len(condition)
            or has_query_top_level_operator(condition)
        ):
        ...
        for target in application.targets:
            ...
            if created:
                ...
                if set_document_value(
                    doc, target.concrete_path, unique_values
                ):
                    modified = True
                continue
            for candidate in values:
    ...
    *,
    ...
    for application in resolve_array_instruction_applications(
        ...
        for target in application.targets:
            ...
            if not isinstance(current, list):
                raise OperationFailure(
                    '$pull requires the target field to be an array'
                )
            filtered = [
                candidate
                for candidate in current
                if not _pull_candidate_matches(
                    candidate, value, context=context
                )
            ]
    ...
    *,
    ...
    for application in resolve_array_instruction_applications(
        ...
        for target in application.targets:
            ...
            if not isinstance(current, list):
                raise OperationFailure(
                    '$pullAll requires the target field to be an array'
                )
            filtered = [
                candidate
                for candidate in current
    ...
    *,
    ...
    for application in resolve_array_instruction_applications(
        ...
        for target in application.targets:
            ...
            if not isinstance(current, list):
                raise OperationFailure(
                    '$pop requires the target field to be an array'
                )
            if not current:
                continue
