...
class FilteringSpecialOperatorsMixin:
    ...
    def _evaluate_exists(
        cls, doc: dict[str, Any], field: str, expected: bool
    ) -> bool:
    ...
    def _normalize_type_specifier(cls, type_spec: Any) -> tuple[str, ...]:
        ...
        if not isinstance(type_spec, str):
            raise ValueError(
                '$type necesita alias string o codigo entero BSON'
            )
        alias_mapping = {
            'double': ('double',),
            'string': ('string',),
    ...
    def _matches_bson_type(cls, candidate: Any, alias: str) -> bool:
        ...
        if alias == 'double':
            return isinstance(candidate, float) and not isinstance(
                candidate, bool
            )
        if alias == 'decimal':
            return isinstance(candidate, (decimal.Decimal, Decimal128))
        if alias in {'int', 'long'}:
            return isinstance(candidate, int) and not isinstance(
                candidate, bool
            )
        if alias == 'string':
        ...
        if alias == 'javascript':
            return (
                BsonCode is not None
                and isinstance(candidate, BsonCode)
                and candidate.scope is None
            )
        ...
        if aliases is None:
            ...
            for type_spec in type_specs:
                resolved_aliases.update(
                    cls._normalize_type_specifier(type_spec)
                )
    ...
    def _coerce_bitwise_mask(cls, operand: Any) -> int:
        ...
        if isinstance(operand, bool):
            raise ValueError(
                'bitwise query operators do not accept boolean masks'
            )
        ...
        if isinstance(operand, uuid.UUID):
            return int.from_bytes(
                operand.bytes, byteorder='little', signed=False
            )
    ...
    def _coerce_bitwise_candidate(cls, candidate: Any) -> int | None:
        ...
        if isinstance(candidate, uuid.UUID):
            return int.from_bytes(
                candidate.bytes, byteorder='little', signed=False
            )
        return None
    ...
    def _evaluate_bitwise(
        ...
        *,
        ...
        if not candidates:
            return False
        resolved_mask = (
            cls._coerce_bitwise_mask(operand) if mask is None else mask
        )
        for candidate in candidates:
            ...
            if (
                operator == '$bitsAllSet'
                and (candidate_bits & resolved_mask) == resolved_mask
            ):
                return True
            if (
                operator == '$bitsAnySet'
                and (candidate_bits & resolved_mask) != 0
            ):
                return True
            if (
        ...
        for expected in expected_values:
            if isinstance(expected, dict) and set(expected) == {'$elemMatch'}:
                if not any(
                    ...
                    return False
                continue
            lookup_key = cls._hashable_in_lookup_key(
                expected, collation=collation
            )
            if lookup_key is not None:
                if literal_lookup is None:
                    literal_lookup = {
                        ...
                        for candidate in candidates
                        if (
                            candidate_key := cls._hashable_in_lookup_key(
                                candidate, collation=collation
                            )
                        )
                        is not None
                    }
            ...
            if not any(
                cls._values_equal(
                    candidate, expected, dialect=dialect, collation=collation
                )
                for candidate in candidates
            ):
                return False
    ...
    @classmethod
    def _evaluate_size(
        cls, doc: dict[str, Any], field: str, expected_size: int
    ) -> bool:
        found, value = cls._get_field_value(doc, field)
        if found:
    ...
    def _evaluate_mod(
        ...
        return any(
            ...
            and divisor != 0
            and cls._mod_remainder_matches(
                cls._mongo_remainder(value, divisor), remainder
            )
            for value in values
        )

    @classmethod
    def _mod_remainder_matches(
        cls, actual: int | float, expected: int | float
    ) -> bool:
    ...
    @classmethod
    def _mongo_remainder(
        cls, value: int | float, divisor: int | float
    ) -> int | float:
        if (
    ...
    @classmethod
    def _evaluate_regex(
        cls, doc: dict[str, Any], field: str, pattern: str, options: str
    ) -> bool:
        regex = _compile_regex(pattern, options)
        return any(
            isinstance(value, str) and regex.search(value) is not None
            for value in cls._extract_values(doc, field)
        )
        ...
        if operator_keys:
            if len(operator_keys) != len(condition):
                raise OperationFailure(
                    '$elemMatch cannot mix operator and field conditions'
                )
