...
def normalize_public_operation_arguments(
    ...
    *,
    ...
    for alias, canonical in aliases.items():
        ...
        if (
            canonical in kwargs
            or explicit.get(canonical, ARG_UNSET) is not ARG_UNSET
        ):
    ...
    for required in spec.required_arguments:
        if normalized.get(required, ARG_UNSET) is ARG_UNSET:
            raise TypeError(
                f"{spec.name}() missing required argument: '{required}'"
            )
...
_FILTER_UPDATE_ALIAS = MappingProxyType(
    {'filter': 'filter_spec', 'update': 'update_spec'}
)
_AGGREGATE_ALIAS = MappingProxyType({'allowDiskUse': 'allow_disk_use'})
...
def normalize_aggregate_operation_arguments(
    operation: str,
    *,
    pipeline: object,
...
def _assert_catalog_alignment(
    operation: str, spec: PublicOperationSpec
) -> None:
...
COLLECTION_DISTINCT_SPEC = PublicOperationSpec(
    name='distinct',
    allowed_options=_options(
        'key', 'filter_spec', _READ_COMMAND_OPTIONS, 'session'
    ),
    aliases=_FILTER_ALIAS,
