...
class SQLiteQueryTranslationTests(unittest.TestCase):
    def test_regex_and_json_each_helpers_cover_safe_and_rejected_shapes(self):
        self.assertEqual(
            parse_safe_literal_regex('^Ada.*', ''), ('prefix', 'Ada', False)
        )
        self.assertEqual(
            parse_safe_literal_regex('Ada$', 'i'), ('suffix', 'Ada', True)
        )
        self.assertEqual(
            parse_safe_literal_regex('Ada', ''), ('contains', 'Ada', False)
        )
        self.assertEqual(
            parse_safe_literal_regex('^Ada$', ''), ('exact', 'Ada', False)
        )
        self.assertEqual(
            parse_safe_literal_regex('Ada\\.', ''), ('contains', 'Ada.', False)
        )
        ...
        self.assertIsNone(
            _translate_json_each_scalar_match('$.items', object())
        )
        with self.assertRaisesRegex(
            NotImplementedError,
            'Unsupported comparison value for SQL translation',
        ):
    ...
    def test_all_elem_match_and_type_translation_cover_error_branches(self):
        with self.assertRaisesRegex(
            NotImplementedError, 'Only top-level \\$all paths'
        ):
            _translate_all_condition('items.name', ('Ada',))
        with self.assertRaisesRegex(
            NotImplementedError, 'Empty \\$all values'
        ):
        ...
        with self.assertRaisesRegex(
            NotImplementedError, 'simple scalar values'
        ):
            _translate_all_condition(
                'items', (ObjectId('0123456789abcdef01234567'),)
            )

        self.assertIn(
            "json_extract(document, '$.items')",
            _translate_all_condition('items', ('Ada',))[0],
        )
        with self.assertRaisesRegex(
            NotImplementedError, 'Only top-level \\$elemMatch paths'
        ):
            _translate_elem_match_condition(
                'items.value', EqualsCondition('value', 1), wrap_value=True
            )
        with self.assertRaisesRegex(
            NotImplementedError, 'Only scalar \\$elemMatch shapes'
        ):
        ...
        with self.assertRaisesRegex(
            ...
            _translate_elem_match_condition(
                'items', EqualsCondition('value', 1), wrap_value=False
            )
        with self.assertRaisesRegex(
            NotImplementedError, 'Unsupported \\$elemMatch scalar predicate'
        ):
            _translate_elem_match_condition(
                'items', MatchAll(), wrap_value=True
            )
    ...
    def test_json_each_value_plan_and_array_aware_comparison_cover_additional_paths(
        self,
    ):
        self.assertIsNone(
            _translate_json_each_value_plan(EqualsCondition('other', 1))
        )
        self.assertIsNone(
            _translate_json_each_value_plan(
                EqualsCondition('value', ObjectId('0123456789abcdef01234567'))
            )
        )
        self.assertEqual(
        ...
        self.assertEqual(
            _translate_json_each_value_plan(
                RegexCondition('value', '^Ada$', '')
            ),
            ("json_each.type = 'text' AND json_each.value = ?", ['Ada']),
        )
        self.assertIsNone(
            _translate_json_each_value_plan(
                RegexCondition('value', '^Áda$', 'i')
            )
        )
        self.assertIsNone(
            _translate_json_each_value_plan(RegexCondition('value', 'A.*', ''))
        )
        self.assertEqual(
            _translate_json_each_value_plan(
                GreaterThanCondition('value', 'Ada')
            ),
            ("json_each.type = 'text' AND json_each.value > ?", ['Ada']),
        )
        self.assertEqual(
            _translate_json_each_value_plan(
                LessThanOrEqualCondition('value', False)
            ),
            (
                "json_each.type IN ('true', 'false') AND json_each.value <= ?",
                [0],
            ),
        )
        self.assertEqual(
            _translate_json_each_value_plan(
                GreaterThanOrEqualCondition('value', 4)
            ),
            (
                "json_each.type IN ('integer', 'real') AND json_each.value >= ?",
                [4],
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            (
                "json_each.type = 'text' AND substr(json_each.value, 1, length(?)) = ?",
                ['Ada', 'Ada'],
            ),
        )
        self.assertEqual(
            ...
            (
                "json_each.type = 'text' AND substr(json_each.value, -length(?)) = ?",
                ['Ada', 'Ada'],
            ),
        )
        self.assertEqual(
            ...
            (
                "json_each.type = 'text' AND instr(json_each.value, ?) > 0",
                ['Ada'],
            ),
        )
        with self.assertRaisesRegex(
    ...
    def test_sqlite_query_private_translation_helpers_cover_none_fallbacks_and_assert_never(
        self,
    ):
        sql, params = _translate_array_contains_scalar(
            'items', ObjectId('0123456789abcdef01234567')
        )
        ...
        self.assertIsNone(
            _translate_json_each_value_plan(
                NotCondition(EqualsCondition('value', 1))
            )
        )
        self.assertIsNone(
            _translate_json_each_value_plan(
                RegexCondition('value', '^Ada$', 'm')
            )
        )
    ...
    def test_sqlite_query_private_translation_helpers_cover_remaining_none_and_error_branches(
        self,
    ):
        self.assertIsNone(parse_safe_literal_regex('^$', ''))
        self.assertIsNone(parse_safe_literal_regex('', ''))
        with patch(
            'mongoeco.engines.sqlite_query._translate_json_each_scalar_match',
            return_value=('bad', []),
        ):
            self.assertIsNone(
                _translate_json_each_value_plan(EqualsCondition('value', 1))
            )
        with patch(
            'mongoeco.engines.sqlite_query.parse_safe_literal_regex',
            return_value=('mystery', 'Ada', False),
        ):
            self.assertIsNone(
                _translate_json_each_value_plan(
                    RegexCondition('value', 'Ada', '')
        ...
        with self.assertRaisesRegex(
            NotImplementedError, 'Unsupported array-aware comparison value'
        ):
            _translate_scalar_or_array_same_type_comparison(
                '>', 'items', ObjectId('0123456789abcdef01234567')
            )
    ...
    def test_path_crosses_scalar_parent_detects_scalar_none_list_and_missing_cases(
        self,
    ):
        self.assertFalse(
            _path_crosses_scalar_parent({'profile': None}, 'profile.name')
        )
        self.assertTrue(
            _path_crosses_scalar_parent({'profile': 1}, 'profile.name')
        )
        self.assertTrue(
            _path_crosses_scalar_parent({'items': []}, 'items.name.more')
        )
        self.assertFalse(
            _path_crosses_scalar_parent({'items': []}, 'items.0.name')
        )
        self.assertFalse(
            _path_crosses_scalar_parent({'items': [None]}, 'items.0.name')
        )
        self.assertFalse(
            _path_crosses_scalar_parent(
                {'items': [{'profile': {}}]}, 'items.0.profile.name'
            )
        )
        self.assertTrue(
            _path_crosses_scalar_parent({'items': [1]}, 'items.0.name')
        )
    ...
    def test_json_path_for_field_supports_dot_and_numeric_segments(self):
        ...
        self.assertEqual(
            json_path_for_field('items.0.name'), '$.items[0].name'
        )
    ...
    def test_translate_query_plan_covers_type_alias_and_or_paths(self):
        ...
        sql, params = translate_query_plan(
            OrCondition((EqualsCondition('a', 1), EqualsCondition('b', 2)))
        )
        self.assertIn(' OR ', sql)
        self.assertEqual(params, [1, 1, 2, 2])

    def test_translate_json_schema_condition_is_explicitly_deferred_to_python(
        self,
    ):
        with self.assertRaisesRegex(
            NotImplementedError, 'not yet translated to SQL'
        ):
            translate_query_plan(
                JsonSchemaCondition(
                    {
    ...
    def test_translate_equals_and_not_equals(self):
        ...
        self.assertEqual(
            translate_query_plan(
                EqualsCondition('name', None, null_matches_undefined=True)
            ),
            (
    ...
    def test_translate_type_and_bitwise_conditions(self):
        sql, params = translate_query_plan(
            TypeCondition('value', ('number', 'string'))
        )
        self.assertIn(
            "json_type(document, '$.value') IN ('integer', 'real')", sql
        )
    ...
    def test_translate_type_condition_supports_numeric_codes_and_aliases(self):
        sql, _ = translate_query_plan(
            TypeCondition('value', (1, 3, 5, 7, 8, 9, 10, 16, 18, 'undefined'))
        )
        ...
        self.assertIn(
            "json_type(document, '$.value') IN ('true', 'false')", sql
        )
    ...
    def test_internal_translation_helpers_cover_string_and_error_branches(
        self,
    ):
    ...
    def test_translate_comparison_rejects_non_comparable_tagged_values(self):
        self.assertEqual(
            _normalize_comparable_value(UNDEFINED), ('undefined', True)
        )

        with self.assertRaises(NotImplementedError):
            _translate_comparison('>', 'payload', {'nested': True})

    def test_sqlite_query_helper_branches_cover_same_type_comparison_type_codes_and_update_fallbacks(
        self,
    ):
        ...
        self.assertIn(
            "json_type(document, '$.flag') IN ('true', 'false')", sql
        )
        self.assertEqual(params, [1])

        sql, params = _translate_same_type_comparison(
        ...
        with mock.patch.object(
            UpdateEngine, 'compile_update_plan', return_value=object()
        ):
            with self.assertRaisesRegex(
                NotImplementedError,
                'Aggregation pipeline updates require Python update fallback',
            ):
                translate_update_spec({'$set': {'name': 'Ada'}})

        with self.assertRaisesRegex(
            NotImplementedError, 'Unsupported update operator'
        ):
            translate_compiled_update_plan(
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            'Array index paths require Python update fallback',
        ):
            translate_compiled_update_plan(
                UpdateEngine.compile_update_plan(
                    {'$set': {'items.0.name': 'Ada'}}
                )
            )
        with self.assertRaisesRegex(
            NotImplementedError,
            'Scalar parent requires Python update fallback',
        ):
            translate_compiled_update_plan(
                UpdateEngine.compile_update_plan(
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            'DBRef special subfields require Python query fallback',
        ):
    ...
    def test_translate_codec_aware_equals(self):
        ...
        sql, params = translate_query_plan(
            EqualsCondition('created_at', created_at)
        )
        self.assertIn("json_each.type = 'object'", sql)
        self.assertEqual(
            params,
        ...
        sql, params = translate_query_plan(
            EqualsCondition('session_id', session_id)
        )
        self.assertIn("json_each.type = 'object'", sql)
        self.assertEqual(
            params,
        ...
        sql, params = translate_query_plan(
            EqualsCondition('legacy', UNDEFINED)
        )
        self.assertIn("json_each.type = 'object'", sql)
        self.assertEqual(params, ['undefined', True, 'undefined', True])

        sql, params = translate_query_plan(
            NotEqualsCondition('_id', object_id)
        )
    ...
    def test_translate_logical_nodes(self):
        self.assertEqual(translate_query_plan(AndCondition(())), ('1 = 1', []))
        self.assertEqual(translate_query_plan(OrCondition(())), ('0 = 1', []))
        plan = AndCondition(
            (
                EqualsCondition('name', 'Ada'),
                OrCondition(
                    (
                        ExistsCondition('role', True),
                        EqualsCondition('kind', 'user'),
                    )
                ),
            )
        )
    ...
    def test_index_expressions_match_normalized_type_and_value_sql(self):
        ...
        self.assertEqual(
            type_expression_sql('session_id'),
            index_expressions_sql('session_id')[0],
        )
        self.assertEqual(
            value_expression_sql('session_id'),
            index_expressions_sql('session_id')[1],
        )
    ...
    def test_translate_codec_aware_comparison(self):
        ...
        sql, params = translate_query_plan(
            GreaterThanCondition('created_at', created_at)
        )
        self.assertIn(
            "CASE WHEN json_type(document, '$.created_at') IS NULL", sql
        )
        self.assertIn("'datetime'", sql)
    ...
    def test_translate_update_spec(self):
        sql, params = translate_update_spec(
            {'$set': {'name': 'Ada'}, '$unset': {'rank': ''}}
        )

        self.assertEqual(
            sql, "json_remove(json_set(document, '$.name', json(?)), '$.rank')"
        )
        self.assertEqual(params, [json.dumps('Ada')])

        nested_sql, nested_params = translate_update_spec(
            {'$set': {'profile.name': 'Ada'}, '$unset': {'profile.age': ''}}
        )
        self.assertEqual(
            nested_sql,
        ...
        self.assertEqual(
            ordered_sql, "json_set(document, '$.profile', json(?))"
        )
        self.assertEqual(
            ordered_params,
            [
                json.dumps(
                    DocumentCodec.encode({'z': 1, 'a': 2}),
                    separators=(',', ':'),
                    sort_keys=False,
                )
            ],
    ...
    def test_translate_update_spec_rejects_unsupported_payloads_and_paths(
        self,
    ):
        with self.assertRaises(NotImplementedError):
