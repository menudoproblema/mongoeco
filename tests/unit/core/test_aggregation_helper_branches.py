...
from mongoeco.core.aggregation import (
    control_object_expressions as control_module,
)
...
class ArrayStringHelperBranchTests(unittest.TestCase):
    def test_set_lookup_helpers_cover_hashable_and_residual_paths(self):
        ...
        lookup, residual = array_module._build_set_lookup(
            values, dialect=MONGODB_DIALECT_70
        )
        ...
        self.assertIsNone(
            array_module._hashable_set_lookup_key(
                'name', dialect=MONGODB_DIALECT_80
            )
        )

    def test_array_string_helpers_cover_slice_zip_regex_bounds_and_trim_paths(
        self,
    ):
        ...
        self.assertIsNone(
            evaluate_expression({'items': None}, {'$slice': ['$items', 2]})
        )
        self.assertEqual(
            evaluate_expression(
                {'items': [1, 2, 3]}, {'$slice': ['$items', 0]}
            ),
            [],
        )
        self.assertEqual(
            evaluate_expression(
                {'items': [1, 2, 3]}, {'$slice': ['$items', -2]}
            ),
            [2, 3],
        )
        with self.assertRaisesRegex(
            OperationFailure, '\\$slice count must be an integer'
        ):
            evaluate_expression({'items': [1]}, {'$slice': ['$items', 'x']})
        with self.assertRaisesRegex(
            OperationFailure, '\\$slice position must be an integer'
        ):
        ...
        with self.assertRaisesRegex(
            OperationFailure, '\\$slice count must be a non-negative integer'
        ):
            evaluate_expression({'items': [1]}, {'$slice': ['$items', 0, -1]})

        self.assertIsNone(
            evaluate_expression({'doc': None}, {'$objectToArray': '$doc'})
        )
        with self.assertRaisesRegex(
            OperationFailure, '\\$objectToArray requires a document input'
        ):
        ...
        self.assertIsNone(
            evaluate_expression({'a': [[1], None]}, {'$zip': {'inputs': '$a'}})
        )
        with self.assertRaisesRegex(
            OperationFailure, '\\$zip requires inputs'
        ):
            evaluate_expression({}, {'$zip': {}})
        with self.assertRaisesRegex(
            OperationFailure, '\\$zip defaults must evaluate to an array'
        ):
            evaluate_expression(
                {'a': [[1], [2]]},
                {'$zip': {'inputs': '$a', 'defaults': 'bad'}},
            )
        with self.assertRaisesRegex(
            OperationFailure, '\\$zip defaults length must match inputs length'
        ):
            evaluate_expression(
                {'a': [[1], [2]]}, {'$zip': {'inputs': '$a', 'defaults': [0]}}
            )
        self.assertIsNone(
            evaluate_expression({'a': None}, {'$zip': {'inputs': '$a'}})
        )

        self.assertIsNone(
            evaluate_expression(
                {'text': None}, {'$indexOfBytes': ['$text', 'a']}
            )
        )
        self.assertEqual(
            evaluate_expression(
                {'text': 'abcd'}, {'$indexOfBytes': ['$text', 'a', 5, 6]}
            ),
            -1,
        )
        self.assertEqual(
            evaluate_expression(
                {'text': 'abcd'}, {'$indexOfCP': ['$text', 'a', 5, 6]}
            ),
            -1,
        )
        self.assertEqual(
            evaluate_expression(
                {'text': 'abcd'}, {'$indexOfBytes': ['$text', 'a', 3, 1]}
            ),
            -1,
        )
        self.assertEqual(
            evaluate_expression(
                {'text': 'abcd'}, {'$indexOfCP': ['$text', 'a', 3, 1]}
            ),
            -1,
        )
        with self.assertRaisesRegex(
            OperationFailure, 'start must be an integer'
        ):
            array_module._normalize_index_bounds('$indexOfBytes', 'x', None, 5)
        with self.assertRaisesRegex(
            OperationFailure, 'end must be an integer'
        ):
            array_module._normalize_index_bounds('$indexOfBytes', 0, 'x', 5)
        with self.assertRaisesRegex(
            OperationFailure, 'start must be a non-negative integer'
        ):
            array_module._normalize_index_bounds('$indexOfBytes', -1, None, 5)
        with self.assertRaisesRegex(
            OperationFailure, 'end must be a non-negative integer'
        ):
        ...
        self.assertIsNone(
            evaluate_expression(
                {'text': 'Ada'},
                {'$regexFind': {'input': '$text', 'regex': 'z'}},
            )
        )
        with self.assertRaisesRegex(
            OperationFailure, 'requires input and regex'
        ):
            evaluate_expression(
                {'text': 'Ada'}, {'$regexFind': {'input': '$text'}}
            )
        self.assertEqual(
            array_module._compile_aggregation_regex(
                Regex('^ada', 'i'), None, operator='$regexMatch'
            ).flags
            & re.IGNORECASE,
            re.IGNORECASE,
        )
        with self.assertRaisesRegex(
            ...
            array_module._compile_aggregation_regex(
                Regex('^ada', 'i'), 'm', operator='$regexMatch'
            )
        with self.assertRaisesRegex(
            OperationFailure, 'only support embedded i and m options'
        ):
            array_module._compile_aggregation_regex(
                re.compile('a', re.DOTALL), None, operator='$regexMatch'
            )
        compiled_pattern = array_module._compile_aggregation_regex(
            re.compile('a', re.IGNORECASE | re.MULTILINE),
            None,
            operator='$regexMatch',
        )
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'Unsupported regex option'
        ):
            array_module._compile_aggregation_regex(
                'a', 'q', operator='$regexMatch'
            )

        self.assertEqual(array_module._substr_code_points('abcd', -1, 2), '')
        self.assertEqual(array_module._substr_code_points('abcd', 10, 2), '')
        self.assertEqual(
            array_module._trim_string('\x00  Ada \x00', None, mode='both'),
            'Ada',
        )
        self.assertEqual(
            array_module._trim_string('--Ada--', '-', mode='left'), 'Ada--'
        )
        self.assertEqual(
            array_module._trim_string('--Ada--', '-', mode='right'), '--Ada'
        )
        with self.assertRaisesRegex(
            OperationFailure, 'Unsupported array/string expression operator'
        ):
            array_module.evaluate_array_string_expression(
                ...
                evaluate_expression_with_missing=lambda _doc, expr, _vars=None: (
                    expr
                ),
...
class DateHelperBranchTests(unittest.TestCase):
    def test_date_helper_branches_cover_formats_parts_timezones_and_units(
        self,
    ):
        self.assertEqual(
        ...
        self.assertEqual(
            date_module._python_strptime_format(
                '$dateFromString', '%Y-%m-%d %H:%M:%S.%L%z'
            ),
            '%Y-%m-%d %H:%M:%S.%f%z',
        )
        with self.assertRaisesRegex(
            OperationFailure, 'format token is not supported'
        ):
            date_module._python_strptime_format('$dateFromString', '%Q')

        eval_expr = lambda _doc, expr, _vars=None: expr
        with self.assertRaisesRegex(
            OperationFailure, 'requires either year or isoWeekYear'
        ):
            date_module._build_date_from_parts(
                '$dateFromParts', {}, {}, None, evaluate_expression=eval_expr
            )
        with self.assertRaisesRegex(
            OperationFailure, 'cannot mix calendar and iso week date parts'
        ):
            date_module._build_date_from_parts(
                '$dateFromParts',
                {'year': 2026, 'isoWeekYear': 2026},
                {},
                None,
                evaluate_expression=eval_expr,
            )
        with self.assertRaisesRegex(
            OperationFailure, 'year must be specified'
        ):
            date_module._build_date_from_parts(
                '$dateFromParts',
                {'month': 1},
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'isoWeekYear must be specified'
        ):
            date_module._build_date_from_parts(
                '$dateFromParts',
                {'isoWeek': 1},
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'produced an invalid date'
        ):
            date_module._build_date_from_parts(
                '$dateFromParts',
        ...
        self.assertEqual(
            date_module._resolve_timezone('$x', '-02:30').utcoffset(None),
            datetime.timedelta(hours=-2, minutes=-30),
        )
        madrid = date_module._resolve_timezone('$x', 'Europe/Madrid')
        self.assertIsNotNone(madrid)
        with self.assertRaisesRegex(
            OperationFailure, 'timezone must evaluate to a string'
        ):
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires a date expression'
        ):
            date_module._evaluate_localized_date_operand(
                '$year',
                {},
        ...
        self.assertEqual(
            date_module._add_date_unit(base, 'millisecond', 1),
            base + datetime.timedelta(milliseconds=1),
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'second', 1),
            base + datetime.timedelta(seconds=1),
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'minute', 1),
            base + datetime.timedelta(minutes=1),
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'hour', 1),
            base + datetime.timedelta(hours=1),
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'day', 1),
            base + datetime.timedelta(days=1),
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'week', 1),
            base + datetime.timedelta(weeks=1),
        )
        self.assertEqual(date_module._add_date_unit(base, 'month', 1).month, 2)
        self.assertEqual(
            date_module._add_date_unit(base, 'quarter', 1).month, 4
        )
        self.assertEqual(
            date_module._add_date_unit(base, 'year', 1).year, 2027
        )
        with self.assertRaisesRegex(OperationFailure, 'Unsupported date unit'):
            date_module._add_date_unit(base, 'century', 1)
        with self.assertRaisesRegex(
            OperationFailure, 'unit must evaluate to a string'
        ):
        ...
        self.assertEqual(
            date_module._date_diff_units(
                start, end, 'millisecond', start_of_week='sunday'
            ),
            90061001,
        )
        self.assertEqual(
            date_module._date_diff_units(
                start, end, 'second', start_of_week='sunday'
            ),
            90061,
        )
        self.assertEqual(
            date_module._date_diff_units(
                start, end, 'minute', start_of_week='sunday'
            ),
            1501,
        )
        self.assertEqual(
            date_module._date_diff_units(
                start, end, 'year', start_of_week='sunday'
            ),
            0,
        )
        with self.assertRaisesRegex(OperationFailure, 'unit is invalid'):
            date_module._date_diff_units(
                start, end, 'century', start_of_week='sunday'
            )

        with self.assertRaisesRegex(
            OperationFailure, 'Unsupported date expression operator'
        ):
    ...
    def test_date_expression_private_branches_cover_null_invalid_and_on_error_paths(
        self,
    ):
        eval_expr = lambda _doc, expr, _vars=None: expr
        eval_missing = lambda _doc, expr, _vars=None: expr
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires a datetime startDate'
        ):
            date_module.evaluate_date_expression(
                '$dateSubtract',
                {},
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'format must evaluate to a string'
        ):
            date_module.evaluate_date_expression(
                '$dateToString',
                {},
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires a document specification'
        ):
            date_module.evaluate_date_expression(
                '$dateFromParts',
                {},
        ...
        self.assertIsNone(
            date_module.evaluate_date_expression(
                ...
                None,
                evaluate_expression=eval_expr,
                evaluate_expression_with_missing=lambda *_args, **_kwargs: (
                    missing
                ),
                missing_sentinel=missing,
            )
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires startDate, endDate and unit'
        ):
            date_module.evaluate_date_expression(
                '$dateDiff',
                {},
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'could not parse dateString'
        ):
            date_module.evaluate_date_expression(
                '$dateFromString',
                {},
...
class NumericHelperBranchTests(unittest.TestCase):
    def test_numeric_helper_branches_cover_nan_percentiles_rounding_and_subtract(
        self,
    ):
        nan_result = evaluate_expression(
            {'value': float('nan')}, {'$abs': '$value'}
        )
        self.assertTrue(math.isnan(nan_result))
        sqrt_nan = evaluate_expression(
            {'value': float('nan')}, {'$sqrt': '$value'}
        )
        self.assertTrue(math.isnan(sqrt_nan))
        self.assertIsNone(
            evaluate_expression(
                {'left': None, 'right': 2}, {'$pow': ['$left', '$right']}
            )
        )
        decimal_pow = evaluate_expression(
            {'left': BsonDecimal128(decimal.Decimal('2.0')), 'right': 2},
            {'$pow': ['$left', '$right']},
        )
        self.assertEqual(decimal_pow, BsonDecimal128(decimal.Decimal('4.0')))
        self.assertEqual(
            evaluate_expression(
                {'left': BsonDouble(2.0), 'right': 3},
                {'$pow': ['$left', '$right']},
            ),
            BsonDouble(8.0),
        )
        self.assertEqual(
            evaluate_expression(
                {'left': BsonInt32(2), 'right': BsonInt64(3)},
                {'$pow': ['$left', '$right']},
            ),
            BsonDouble(8.0),
        )
        ...
        self.assertEqual(
            numeric_module._sum_accumulator_operand(BsonInt32(3)), BsonInt32(3)
        )
        self.assertEqual(
            numeric_module._stddev_accumulator_operand(UNDEFINED), None
        )
        self.assertEqual(
            numeric_module._stddev_accumulator_operand(decimal.Decimal('1.5')),
            1.5,
        )
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'p must evaluate to an array'
        ):
            numeric_module._normalize_percentile_probabilities(
                '$percentile', 'bad'
            )
        with self.assertRaisesRegex(
            OperationFailure, 'must not be an empty array'
        ):
            numeric_module._normalize_percentile_probabilities(
                '$percentile', []
            )
        ...
        self.assertEqual(
            numeric_module._compute_percentiles([1, 3, 2], [0.0, 1.0]), [1, 3]
        )
        ...
        self.assertEqual(
            numeric_module._require_integral_numeric('$bitNot', 4.0), 4
        )
        with self.assertRaisesRegex(
            OperationFailure, 'requires numeric arguments'
        ):
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires integral numeric arguments'
        ):
        ...
        self.assertEqual(
            numeric_module._subtract_values(when, 1000),
            when - datetime.timedelta(milliseconds=1000),
        )
        self.assertEqual(
            numeric_module._subtract_values(
                when, when - datetime.timedelta(seconds=2)
            ),
            2000,
        )
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'Unsupported numeric expression operator'
        ):
...
class AccumulatorHelperBranchTests(unittest.TestCase):
    def test_accumulator_helper_branches_cover_flags_validation_and_finalize_paths(
        self,
    ):
        bucket = {}
        created_flags = accumulators_module._accumulator_flags(bucket)
        self.assertIs(
            created_flags, bucket[accumulators_module._ACCUMULATOR_FLAGS_KEY]
        )
        self.assertEqual(
            accumulators_module._accumulator_flags(
                accumulators_module._AccumulatorBucket('a', {})
            ),
            {},
        )
        self.assertEqual(
            accumulators_module._accumulator_flags(
                {accumulators_module._ACCUMULATOR_FLAGS_KEY: {'seen': True}}
            ),
            {'seen': True},
        )
        ...
        for operator, expression in (
            ...
            with self.subTest(operator=operator):
                with self.assertRaises(OperationFailure):
                    accumulators_module._validate_accumulator_expression(
                        operator, expression
                    )

        self.assertIsNone(accumulators_module._sum_accumulator_operand(['x']))
        self.assertEqual(
            accumulators_module._sum_accumulator_operand([1, 2, 'x']), 3
        )
        self.assertTrue(
            math.isnan(
                accumulators_module._stddev_accumulator_operand(float('inf'))
            )
        )
        ...
        with self.assertRaisesRegex(OperationFailure, 'requires input and n'):
            accumulators_module._evaluate_pick_n_size(
                '$firstN', {}, {}, None, evaluate_expression=eval_expr
            )
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires sortBy and output'
        ):
            accumulators_module._evaluate_ordered_accumulator_input(
                ...
                resolve_aggregation_field_path=lambda doc, field: doc.get(
                    field
                ),
                missing_sentinel=missing,
            )
        ...
        state = accumulators_module._OrderedAccumulator(
            items=[([3], 'c', 1), ([1], 'a', 2), ([2], 'b', 3)]
        )
        accumulators_module._trim_ordered_accumulator(
            state, [('x', 1)], keep=1, bottom=True
        )
        self.assertEqual([item[1] for item in state.items], ['c'])

        self.assertEqual(
            accumulators_module._coerce_accumulator_specs(None),
            (('count', '$sum', 1),),
        )
        prepared = (('a', '$sum', 1),)
        self.assertIs(
            accumulators_module._coerce_accumulator_specs(prepared), prepared
        )
        with self.assertRaisesRegex(OperationFailure, 'single-key document'):
            accumulators_module._coerce_accumulator_specs(
                {'a': {'$sum': 1, '$max': 2}}
            )

        std_bucket = {
            'score': accumulators_module._StdDevAccumulator(population=True)
        }
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'consistent positive integer'
        ):
            accumulators_module._apply_accumulators(
                {
                    'top': accumulators_module._OrderedAccumulator(
                        n=2, sort_spec=[('rank', 1)]
                    )
                },
                {
                    'top': {
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'consistent positive integer within the group'
        ):
            accumulators_module._apply_accumulators(
                {
                    'firstOne': accumulators_module._PickNAccumulator(
                        items=[1], n=1
                    )
                },
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'consistent array within the group'
        ):
            accumulators_module._apply_accumulators(
                {
                    'pct': accumulators_module._PercentileAccumulator(
                        probabilities=[0.5]
                    )
                },
                {
        ...
        self.assertFalse(
            accumulators_module._window_sort_keys_equal([1], [1, 2])
        )

        finalized = accumulators_module._finalize_accumulators(
            {
                'avg': accumulators_module._AverageAccumulator(
                    total=decimal.Decimal('9'), count=2
                ),
                ...
                'ordered_scalar': accumulators_module._OrderedAccumulator(
                    items=[([1], 'x', 0)]
                ),
                'ordered_list': accumulators_module._OrderedAccumulator(
                    items=[([1], 'x', 0), ([2], 'y', 1)], n=2
                ),
                'percentile_scalar': accumulators_module._PercentileAccumulator(
                    values=[1, 5], probabilities=[0.5], scalar_output=True
                ),
                'percentile_list': accumulators_module._PercentileAccumulator(
                    values=[], probabilities=[0.5], scalar_output=False
                ),
            }
        )
...
class CompiledAggregationHelperBranchTests(unittest.TestCase):
    def test_compiled_group_private_helpers_cover_cache_freezing_and_simple_arithmetic(
        self,
    ):
        ...
        self.assertFalse(
            compiled_module.CompiledGroup.supports(
                {'total': {'$sum': '$value'}}
            )
        )
        self.assertEqual(
        ...
        frozen_regex = compiled_module.CompiledGroup._freeze_cache_value(
    ...
    def test_compiled_group_private_branches_cover_cache_eviction_and_expression_codegen(
        self,
    ):
        original_maxsize = (
            compiled_module.CompiledGroup._COMPILED_FUNCTION_CACHE_MAXSIZE
        )
        ...
        try:
            compiled_module.CompiledGroup(
                {'_id': '$kind', 'total': {'$sum': '$value'}}
            )
            compiled_module.CompiledGroup(
                {'_id': '$other', 'total': {'$sum': '$value'}}
            )
            self.assertEqual(
                len(compiled_module.CompiledGroup._COMPILED_FUNCTION_CACHE), 1
            )
        finally:
            compiled_module.CompiledGroup._COMPILED_FUNCTION_CACHE.clear()
            compiled_module.CompiledGroup._COMPILED_FUNCTION_CACHE_MAXSIZE = (
        ...
        compiled = compiled_module.CompiledGroup(
            {'_id': '$kind', 'total': {'$sum': '$value'}}
        )
        self.assertEqual(compiled._compile_expression('plain', 'x'), "'plain'")
        self.assertEqual(
            compiled._compile_expression({'$literal': 'value'}, 'x'), "'value'"
        )
        self.assertEqual(
            compiled._compile_expression({'nested': True}, 'x').startswith(
                '_evaluate('
            ),
            True,
        )
...
class PlanningAndJoinHelperBranchTests(unittest.TestCase):
    def test_planning_and_join_helpers_cover_documents_stage_and_lookup_let_validation(
        self,
    ):
        with self.assertRaisesRegex(
            OperationFailure, '\\$documents requires an array of documents'
        ):
            planning_module._require_documents_stage('bad')

        with self.assertRaisesRegex(
            OperationFailure,
            'must begin with a lowercase letter or non-ascii character',
        ):
...
class ControlObjectHelperBranchTests(unittest.TestCase):
    def test_control_object_private_branches_cover_invalid_specs_and_fallbacks(
        self,
    ):
        ...
        self.assertIsNone(
            control_module._evaluate_field_bound_match_expression(
                '$and', document, ['$items', []]
            )
        )
        self.assertIsNone(
        ...
        self.assertIsNone(
            control_module._evaluate_field_bound_query_operator(
                '$in', document, 'bad'
            )
        )
        self.assertIsNone(
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'only supports the field-bound form'
        ):
            ...
            with self.subTest(operator=operator):
                with self.assertRaisesRegex(OperationFailure, expected):
                    control_module.evaluate_control_object_expression(
                        ...
                        evaluate_expression_with_missing=eval_missing,
                        require_expression_args=require_args,
                        compare_values=lambda left, right, operator: (
                            left == right
                        ),
                        expression_truthy=bool,
                        require_array=lambda _op, value: value,
                        evaluate_pick_n_input=lambda *_args, **_kwargs: (
                            [],
                            1,
                        ),
                        missing_sentinel=object(),
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'no default was specified'
        ):
            control_module.evaluate_control_object_expression(
                '$switch',
                document,
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'requires at least 1 arguments'
        ):
            control_module.evaluate_control_object_expression(
                '$mergeObjects',
                document,
        ...
        self.assertIsNone(
            control_module.evaluate_control_object_expression(
                ...
                None,
                evaluate_expression=lambda _doc, expr, _vars=None: (
                    None if expr is None else document
                ),
                evaluate_expression_with_missing=lambda _doc, expr, _vars=None: (
                    document
                ),
        ...
        with self.assertRaisesRegex(
            OperationFailure, 'Unsupported control/object expression operator'
        ):
            control_module.evaluate_control_object_expression(
                '$unknown',
                document,
