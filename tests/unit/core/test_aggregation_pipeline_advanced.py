...
class AggregationPipelineAdvancedTests(unittest.TestCase):
    def test_pipeline_supports_array_to_object_index_of_array_and_sort_array(
        self,
    ):
        documents = [
            {
    ...
    def test_pipeline_supports_facet_and_date_trunc(self):
        documents = [
            ...
            },
            {
                ...
                'kind': 'view',
                'score': 7,
                'created_at': datetime.datetime(2026, 3, 24, 10, 5, 0),
            },
            {
                '_id': '3',
                'kind': 'click',
                'score': 3,
                'created_at': datetime.datetime(2026, 3, 24, 11, 10, 0),
            },
        ]
    ...
    def test_pipeline_supports_bucket_default_count_output(self):
        ...
        self.assertEqual(
            result, [{'_id': 0, 'count': 1}, {'_id': 10, 'count': 1}]
        )
    ...
    def test_pipeline_supports_set_window_fields_without_explicit_window(self):
        result = apply_pipeline(
            [
                {'_id': '1', 'tenant': 'a', 'rank': 2, 'score': 7},
                {'_id': '2', 'tenant': 'a', 'rank': 1, 'score': 5},
            ],
            [
                {
                    '$setWindowFields': {
    ...
    def test_pipeline_set_window_fields_distinguishes_bool_and_int_partitions(
        self,
    ):
        result = apply_pipeline(
            [
    ...
    def test_pipeline_supports_set_window_fields_with_numeric_range_window(
        self,
    ):
        documents = [
            {'_id': '1', 'score': 5},
    ...
    def test_pipeline_set_window_fields_ignores_non_numeric_values_for_sum(
        self,
    ):
        documents = [
            {'_id': '1', 'tenant': 'a', 'rank': 1, 'score': 5},
    ...
    def test_pipeline_supports_set_window_fields_with_current_and_unbounded_range(
        self,
    ):
        result = apply_pipeline(
    ...
    def test_pipeline_supports_count_and_sort_by_count(self):
        ...
        self.assertEqual(
            sorted_counts,
            [{'_id': 'click', 'count': 2}, {'_id': 'view', 'count': 2}],
        )
    ...
    def test_pipeline_count_returns_empty_result_for_empty_input(self):
        self.assertEqual(
            apply_pipeline([], [{'$count': 'total'}]), [{'total': 0}]
        )
    ...
    def test_pipeline_group_preserves_user_document_that_looks_like_avg_state(
        self,
    ):
        result = apply_pipeline(
        ...
        self.assertEqual(
            result, [{'_id': None, 'firstPayload': {'total': 5, 'count': 2}}]
        )

    def test_pipeline_union_with_current_collection_keeps_empty_resolved_collection_empty(
        self,
    ):
        result = apply_pipeline(
            [{'_id': 'seed'}],
            ...
            collection_resolver=lambda name: (
                [] if name == '__mongoeco_current_collection__' else None
            ),
        )

        self.assertEqual(result, [{'_id': 'seed'}])

    def test_pipeline_union_with_current_collection_falls_back_to_input_when_resolver_returns_none(
        self,
    ):
        result = apply_pipeline(
            [{'_id': 'seed'}],
    ...
    def test_pipeline_set_window_fields_preserves_user_document_that_looks_like_avg_state(
        self,
    ):
        result = apply_pipeline(
            ...
            [
                {
                    '_id': '1',
                    'rank': 1,
                    'payload': {'total': 5, 'count': 2},
                    'firstPayload': {'total': 5, 'count': 2},
                }
            ],
        )
    ...
    def test_pipeline_rejects_invalid_group_and_expression_payloads(self):
        cases = (
            ...
            (
                [{'_id': '1'}],
                [{'$group': {'_id': None, 'total': {'$unknown': 1}}}],
            ),
            ...
            (
                [{'score': 1}],
                [{'$bucket': {'groupBy': '$score', 'boundaries': [10]}}],
            ),
            (
                [{'score': 1}],
                [{'$bucket': {'groupBy': '$score', 'boundaries': [10, 5]}}],
            ),
            (
                [{'score': 1}],
                [
            ...
            (
                [{'score': 50}],
                [{'$bucket': {'groupBy': '$score', 'boundaries': [0, 10]}}],
            ),
            ([{'score': 1}], [{'$bucketAuto': []}]),
            (
                [{'score': 1}],
                [{'$bucketAuto': {'groupBy': '$score', 'buckets': 0}}],
            ),
            (
                [{'score': 1}],
                [
            ...
            (
                [{'_id': '1'}],
                [{'$setWindowFields': {'output': {1: {'$sum': 1}}}}],
            ),
    ...
    def test_split_pushdown_pipeline_keeps_match_with_or_expr_and_sibling_fields_in_core(
        self,
    ):
        match_spec = {
            '$or': [
    ...
    def test_first_n_raises_when_n_evaluates_to_different_value_within_group(
        self,
    ):
        documents = [
    ...
    def test_top_uses_insertion_order_as_tiebreaker_for_equal_sort_values(
        self,
    ):
        documents = [
    ...
    def test_compiled_group_matches_slow_path_when_add_operand_is_missing(
        self,
    ):
        spec = {
            '_id': '$kind',
            'total': {'$sum': {'$add': ['$a', '$missing_field']}},
        }
        documents = [
            {'kind': 'x', 'a': 5},
    ...
    def test_compiled_group_matches_slow_path_for_constant_numeric_sum_literals(
        self,
    ):
    ...
    def test_setwindowfields_range_window_includes_only_docs_within_numeric_bounds(
        self,
    ):
        documents = [
            {'_id': '1', 'value': 1},
