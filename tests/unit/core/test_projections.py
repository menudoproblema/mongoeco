...
from mongoeco.core.projections import (
    _apply_meta_projection,
    apply_projection,
...
class ProjectionTests(unittest.TestCase):
    ...
    def test_inclusion_projection(self):
        doc = {
            ...
            "name": "Val",
            "age": 30,
            "profile": {"city": "Madrid", "job": "Dev"},
        }

        self.assertEqual(
            apply_projection(doc, {"name": 1}),
            {"_id": 1, "name": "Val"},
        )
        self.assertEqual(
    ...
    def test_inclusion_projection_supports_dot_notation_through_list_of_documents(
        ...
        doc = {
            "_id": 1,
            "items": [{"name": "Ada", "age": 1}, {"name": "Grace", "age": 2}],
        }
    ...
    def test_exclusion_projection_supports_dot_notation_through_list_of_documents(
        self,
    ):
        doc = {
            "_id": 1,
    ...
    def test_id_projection(self):
        ...
        self.assertEqual(
            apply_projection(doc, {"name": 1, "_id": 0}),
            {"name": "Val"},
        )
    ...
    def test_empty_projection_returns_full_document(self):
        ...
        self.assertEqual(
            projected,
            {"_id": 1, "name": "Ada", "profile": {"city": "Berlin"}},
        )
        self.assertEqual(
            doc,
    ...
    def test_projection_rejects_mixed_inclusion_exclusion_and_invalid_flags(
        self,
    ):
        doc = {"_id": 1, "a": 1, "b": 2}
        ...
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc,
                {"a": 0, "score": {"$meta": "vectorSearchScore"}},
    ...
    def test_projection_supports_meta_text_score(self):
        doc = {"_id": 1, "name": "Ada"}
        set_document_value(doc, TEXT_SCORE_FIELD, 3.0)
    ...
    def test_projection_supports_meta_vector_search_score(self):
        doc = {"_id": 1, "name": "Ada"}
        set_document_value(doc, VECTOR_SEARCH_SCORE_FIELD, 0.99)
    ...
    def test_projection_supports_meta_search_highlights(self):
        highlights = [
            {"path": "title", "text": "Ada", "operator": "text"},
        ]
        doc = {"_id": 1, "name": "Ada"}
        set_document_value(doc, SEARCH_HIGHLIGHTS_METADATA_FIELD, highlights)
    ...
    def test_projection_supports_slice_as_inclusion_style_projection(self):
        ...
        self.assertEqual(
            apply_projection(
                doc,
                {"name": 1, "profile.colors": {"$slice": [1, 2]}, "_id": 0},
            ),
            {"name": "Ada", "profile": {"colors": ["blue", "green"]}},
        )

    def test_projection_supports_elem_match_and_omits_field_when_there_is_no_match(
        self,
    ):
        doc = {
            "_id": 1,
        ...
        self.assertEqual(
            apply_projection(
                doc,
                {
                    "zipcode": 1,
                    "students": {
                        "$elemMatch": {"school": 102, "age": {"$gt": 10}},
                    },
                },
            ),
            {
    ...
    def test_projection_supports_positional_operator_with_selector_filter(
        self,
    ):
        doc = {
            "_id": 1,
        ...
        self.assertEqual(
            apply_projection(
                ...
                selector_filter={
                    "$or": [
                        {"students.school": 100, "name": "Grace"},
                        {"students.school": 102, "name": "Ada"},
                    ],
                },
            ),
            {"students": [{"school": 102, "age": 10}]},
        )

    def test_positional_projection_requires_selector_filter_and_terminal_segment(
        self,
    ):
        ...
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc,
                {"students.$.school": 1},
                selector_filter={"students.school": 102},
            )
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc,
    ...
    def test_projection_rejects_invalid_projection_operator_specs_and_collisions(
        self,
    ):
        doc = {
            "_id": 1,
            "items": [{"kind": "a"}],
            "profile": {"colors": [1, 2, 3]},
        }
        ...
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc,
                {"items": {"$elemMatch": {"kind": "a"}}, "name": 0},
            )  # type: ignore[dict-item]
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc,
                {"profile": {"$slice": 1}, "profile.colors": 1},
            )  # type: ignore[dict-item]

    def test_projection_validation_private_branches_cover_operator_and_slice_edge_cases(
        self,
    ):
        ...
        with self.assertRaises(OperationFailure):
            validate_projection_spec(
                {"field": {"$slice": 1, "$elemMatch": {"x": 1}}},
            )  # type: ignore[dict-item]
        with self.assertRaises(OperationFailure):
            validate_projection_spec({"$": 1})

        self.assertEqual(
            apply_projection(doc, {"missing": {"$slice": 1}}),
            {"_id": 1, "items": "value", "profile": {"colors": [1, 2, 3]}},
        )
        self.assertEqual(
        ...
        self.assertEqual(
            apply_projection(
                {"_id": 1, "items": "not-list"},
                {"items.$": 1},
                selector_filter={"items": "x"},
            ),
            {"_id": 1},
        )
