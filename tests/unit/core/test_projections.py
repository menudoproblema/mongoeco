...
class ProjectionTests(unittest.TestCase):
    ...
    def test_inclusion_projection(self):
        ...
        self.assertEqual(
            apply_projection(doc, {'name': 1}), {'_id': 1, 'name': 'Val'}
        )
        self.assertEqual(
    ...
    def test_id_projection(self):
        ...
        self.assertEqual(
            apply_projection(doc, {'name': 1, '_id': 0}), {'name': 'Val'}
        )
    ...
    def test_excluding_nested_field_keeps_empty_parent_document(self):
        ...
        self.assertEqual(
            apply_projection(doc, {'a.b': 0}), {'_id': 1, 'a': {}}
        )
    ...
    def test_empty_projection_returns_full_document(self):
        ...
        self.assertEqual(
            projected, {'_id': 1, 'name': 'Ada', 'profile': {'city': 'Berlin'}}
        )
        self.assertEqual(
            doc, {'_id': 1, 'name': 'Ada', 'profile': {'city': 'Madrid'}}
        )
    ...
    def test_projection_rejects_mixed_inclusion_exclusion_and_invalid_flags(
        ...
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc, {'a': 0, 'score': {'$meta': 'vectorSearchScore'}}
    ...
    def test_projection_supports_meta_vector_search_score(self):
        ...
        self.assertEqual(
            apply_projection(
                doc, {'score': {'$meta': 'vectorSearchScore'}, '_id': 0}
            ),  # type: ignore[dict-item]
            {'score': 0.99},
        )
    ...
    def test_projection_supports_elem_match_and_omits_field_when_there_is_no_match(
        ...
        self.assertEqual(
            apply_projection(
                doc, {'students': {'$elemMatch': {'school': 102}}}
            ),
            {
                '_id': 1,
        ...
        self.assertEqual(
            apply_projection(
                missing, {'students': {'$elemMatch': {'school': 102}}}
            ),
            {'_id': 2},
        )
    ...
    def test_projection_rejects_invalid_projection_operator_specs_and_collisions(
        ...
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc, {'items': {'$elemMatch': {'kind': 'a'}}, 'name': 0}
            )  # type: ignore[dict-item]
        with self.assertRaises(OperationFailure):
            apply_projection(
                doc, {'profile': {'$slice': 1}, 'profile.colors': 1}
            )  # type: ignore[dict-item]
