...
class QueryPlanTests(unittest.TestCase):
        ...
        class _NoRegexDialect(MongoDialect):
            def supports_query_field_operator(self, name: str) -> bool:
                return (
                    False
                    if name == '$regex'
                    else super().supports_query_field_operator(name)
                )

        with self.assertRaises(OperationFailure):
            compile_filter(
                {'name': {'$regex': '^ad'}},
                dialect=_NoRegexDialect(
                    key='test', server_version='test', label='No Regex'
                ),
            )

    def test_compile_filter_rejects_custom_supported_but_unimplemented_field_operator(
        self,
    ):
    ...
    def test_compile_filter_rejects_custom_supported_but_unimplemented_top_level_operator(
        self,
    ):
        class _FutureTopLevelDialect(MongoDialect):
            def supports_query_top_level_operator(self, name: str) -> bool:
                return (
                    True
                    if name == '$futureTopLevel'
                    else super().supports_query_top_level_operator(name)
                )
    ...
    def test_compile_filter_supports_safe_where_expressions_and_callables(
        self,
    ):
        where_expression_plan = compile_filter(
            {'$where': "this.age >= 18 and this.role == 'admin'"}
        )
        self.assertIsInstance(where_expression_plan, WhereCondition)
        self.assertEqual(
            where_expression_plan.expression,
            "this.age >= 18 and this.role == 'admin'",
        )
    ...
    def test_compile_filter_rejects_unsafe_where_expressions(self):
        with self.assertRaisesRegex(
            ValueError, 'safe local expression subset'
        ):
        ...
        with self.assertRaisesRegex(
            ValueError, 'must be a non-empty string or callable'
        ):
            compile_filter({'$where': ''})

    def test_geo_query_plan_rejects_invalid_local_runtime_shapes(self):
        with self.assertRaisesRegex(
            ValueError, '\\$geoWithin requires a document specification'
        ):
            compile_filter({'location': {'$geoWithin': 'bad'}})
        with self.assertRaisesRegex(
            OperationFailure, 'only supports Polygon or MultiPolygon'
        ):
            compile_filter(
                {
                    'location': {
        ...
        with self.assertRaisesRegex(
            ValueError, '\\$geoIntersects requires a \\$geometry document'
        ):
            compile_filter({'location': {'$geoIntersects': {}}})
        with self.assertRaisesRegex(
            ValueError, '\\$near requires a \\$geometry point'
        ):
            compile_filter({'location': {'$near': {}}})
        with self.assertRaisesRegex(
            OperationFailure,
            '\\$nearSphere.\\$minDistance must be a non-negative finite number',
        ):
            compile_filter(
                {
                    'location': {
    ...
    def test_compile_filter_carries_null_undefined_behavior_in_equals_and_in(
        self,
    ):
        self.assertEqual(
        ...
        self.assertEqual(
            compile_filter(
                {'role': {'$in': [None]}}, dialect=MONGODB_DIALECT_70
            ),
            InCondition('role', (None,), null_matches_undefined=True),
        )
        self.assertEqual(
            compile_filter(
                {'role': {'$nin': [None]}}, dialect=MONGODB_DIALECT_70
            ),
            NotInCondition('role', (None,), null_matches_undefined=True),
        )
        self.assertEqual(
            compile_filter(
                {'role': {'$nin': [None]}}, dialect=MONGODB_DIALECT_80
            ),
            NotInCondition('role', (None,), null_matches_undefined=False),
        )

    def test_compile_filter_returns_and_condition_for_multiple_field_operators(
        self,
    ):
    ...
    def test_compile_filter_supports_all_size_and_regex(self):
        ...
        self.assertEqual(
            compile_filter(
                {'name': {'$regex': Regex('^ad', 'i'), '$options': 'm'}}
            ),
        ...
        self.assertEqual(
            elem_match.compiled_dialect_key, MONGODB_DIALECT_70.key
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            TypeCondition(
                ...
                (13, 14, 15),
                aliases=frozenset(
                    {'javascript', 'symbol', 'javascriptWithScope'}
                ),
            ),
        )
        self.assertIsInstance(
            compile_filter({'$comment': 'trace-only'}), MatchAll
        )
    ...
    def test_query_plan_private_helpers_cover_regex_type_and_bitwise_branches(
        self,
    ):
        compiled = re.compile(
            '^ad', re.IGNORECASE | re.MULTILINE | re.DOTALL | re.VERBOSE
        )
        self.assertEqual(_regex_options_from_pattern(compiled), 'imsx')

        self.assertEqual(_normalize_type_specifier(127), ('maxKey',))
        self.assertEqual(
            _normalize_type_specifier(' number '),
    ...
    def test_compile_filter_private_branches_cover_regex_equals_and_elem_match_fallback(
        self,
    ):
        self.assertEqual(
        ...
        self.assertEqual(
            compile_filter(
                {
                    'name': {
                        '$eq': re.compile('^ad', re.IGNORECASE | re.MULTILINE)
                    }
                }
            ),
            RegexCondition('name', '^ad', 'im'),
    ...
    def test_compile_filter_accepts_richer_top_level_json_schema(self):
        ...
        self.assertEqual(
            compiled.schema['properties']['profile']['properties']['tags'][
                'items'
            ]['minLength'],
            2,
        )
    ...
    def test_compile_filter_supports_local_geo_subset(self):
        ...
        self.assertTrue(
            box_plan.geometry.equals(shapely_box(-1.0, -1.0, 1.0, 1.0))
        )
    ...
    def test_compile_filter_rejects_unsupported_geo_shapes(self):
        ...
        with self.assertRaisesRegex(
            OperationFailure,
            'supports only \\$geometry Polygon or legacy \\$box',
        ):
            compile_filter(
                {'location': {'$geoWithin': {'$center': [[0, 0], 2]}}}
            )
    ...
    def test_compile_filter_supports_broader_local_geo_shapes(self):
        ...
        self.assertTrue(
            intersects_plan.geometry.equals(MultiPoint([(1, 2), (3, 4)]))
        )
    ...
    def test_ensure_query_plan_prefers_explicit_plan_and_defaults_to_match_all(
        self,
    ):
