...
from mongoeco.core.filtering import (
    BSONComparator,
    HANDLED_QUERY_NODE_TYPES,
    QueryEngine,
)
...
class QueryEngineTests(unittest.TestCase):
    def test_query_engine_dispatch_covers_all_concrete_query_nodes(self):
        def _collect_concrete_subclasses(
            base: type[QueryNode],
        ) -> set[type[QueryNode]]:
    ...
    def test_query_engine_uses_custom_dialect_for_equality_membership_and_ranges(
        self,
    ):
    ...
    def test_query_engine_supports_minmax_and_code_bson_types_when_available(
        self,
    ):
        ...
        self.assertTrue(
            QueryEngine.match(document, {'min': {'$type': 'minKey'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'max': {'$type': 'maxKey'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'plainCode': {'$type': 'javascript'}})
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'scopedCode': {'$type': 'javascriptWithScope'}}
            )
        )
        self.assertLess(
            MONGODB_DIALECT_70.policy.compare_values(BsonMinKey(), 1), 0
        )
        self.assertGreater(
            MONGODB_DIALECT_70.policy.compare_values(BsonMaxKey(), 1), 0
        )

    def test_query_engine_hashable_lookup_key_handles_special_scalar_types(
        self,
    ):
        ...
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(None), ('none', None)
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(True), ('bool', True)
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(b'abc'), ('bytes', b'abc')
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(now), ('date', now)
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(uid), ('uuid', uid)
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(oid), ('objectid', oid)
        )
        self.assertEqual(
            QueryEngine._hashable_in_lookup_key(timestamp),
            ('timestamp', timestamp),
        )
    ...
    def test_query_engine_supports_dot_notation(self):
        ...
        self.assertTrue(
            QueryEngine.match(document, {'user.profile.name': 'Ada'})
        )
    ...
    def test_query_engine_handles_unequal_embedded_documents_without_crashing(
        self,
    ):
        document = {'data': {'b': 1}}
    ...
    def test_type_query_distinguishes_bson_int_and_long_by_range(self):
        self.assertTrue(
            QueryEngine.match({'value': 1}, {'value': {'$type': 'int'}})
        )
        self.assertFalse(
            QueryEngine.match({'value': 1}, {'value': {'$type': 'long'}})
        )
        self.assertTrue(
            QueryEngine.match({'value': 1 << 40}, {'value': {'$type': 'long'}})
        )
        self.assertFalse(
            QueryEngine.match({'value': 1 << 40}, {'value': {'$type': 'int'}})
        )

    def test_type_query_accepts_public_bson_classes(self):
        self.assertTrue(
            QueryEngine.match(
                {'value': Binary(b'abc', subtype=4)},
                {'value': {'$type': 'binData'}},
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'value': Decimal128('1.5')}, {'value': {'$type': 'decimal'}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'value': Regex('^ad', 'i')}, {'value': {'$type': 'regex'}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'value': Timestamp(10, 1)}, {'value': {'$type': 'timestamp'}}
            )
        )
        if BsonCode is not None:
            self.assertTrue(
                QueryEngine.match(
                    {'value': BsonCode('function() { return 1; }')},
                    {'value': {'$type': 13}},
                )
            )
            self.assertTrue(
    ...
    def test_query_plan_accepts_public_regex(self):
        self.assertTrue(
            QueryEngine.match({'name': 'Ada'}, {'name': Regex('^ad', 'i')})
        )
    ...
    def test_query_including_null_matches_undefined_in_7_but_not_in_8(self):
        ...
        plan_70 = compile_filter(
            {'value': {'$in': [None]}}, dialect=MONGODB_DIALECT_70
        )
        plan_80 = compile_filter(
            {'value': {'$in': [None]}}, dialect=MONGODB_DIALECT_80
        )
    ...
    def test_query_ne_null_on_missing_field_differs_between_7_and_8(self):
        ...
        self.assertFalse(
            QueryEngine.match(
                document, {'value': {'$ne': None}}, dialect=MONGODB_DIALECT_70
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'value': {'$ne': None}}, dialect=MONGODB_DIALECT_80
            )
        )
    ...
    def test_query_nin_plan_preserves_compiled_null_semantics_across_runtime_dialects(
        self,
    ):
        document = {'value': UNDEFINED}
        plan_70 = compile_filter(
            {'value': {'$nin': [None]}}, dialect=MONGODB_DIALECT_70
        )
        plan_80 = compile_filter(
            {'value': {'$nin': [None]}}, dialect=MONGODB_DIALECT_80
        )

        self.assertFalse(
            QueryEngine.match_plan(
                document, plan_70, dialect=MONGODB_DIALECT_80
            )
        )
        self.assertTrue(
            QueryEngine.match_plan(
                document, plan_80, dialect=MONGODB_DIALECT_70
            )
        )

    def test_query_ne_and_nin_on_missing_field_still_match_non_null_values(
        self,
    ):
        document = {}
        ...
        self.assertTrue(
            QueryEngine.match(
                document, {'value': {'$ne': 1}}, dialect=MONGODB_DIALECT_70
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'value': {'$ne': 1}}, dialect=MONGODB_DIALECT_80
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'value': {'$nin': [1]}}, dialect=MONGODB_DIALECT_70
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'value': {'$nin': [1]}}, dialect=MONGODB_DIALECT_80
            )
        )
    ...
    def test_query_engine_all_matches_direct_array_members_without_considering_array_as_scalar(
        self,
    ):
        document = {'tags': ['python', 'mongodb']}
        ...
        self.assertTrue(
            QueryEngine.match(
                document, {'tags': {'$all': ['python', 'mongodb']}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
    ...
    def test_query_engine_all_uses_single_resolution_for_array_traversal_and_hashable_fast_path(
        self,
    ):
        ...
        with (
            patch.object(
                QueryEngine,
                '_get_field_value',
                side_effect=AssertionError('unexpected fast-path lookup'),
            ),
            patch.object(
                QueryEngine,
                '_values_equal',
                side_effect=AssertionError('unexpected equality fallback'),
            ),
        ):
            self.assertTrue(
                QueryEngine.match(
                    document, {'items.tags': {'$all': [b'a', b'c']}}
                )
            )

    def test_query_engine_supports_dot_notation_through_list_of_documents(
        self,
    ):
        document = {'items': [{'kind': 'a'}, {'kind': 'b'}]}
        self.assertTrue(QueryEngine.match(document, {'items.kind': 'b'}))

    def test_query_engine_matches_exists_and_ne_on_dotted_array_document_path(
        self,
    ):
        ...
        self.assertEqual(
            QueryEngine.extract_values(document, 'items.exposed_at'),
            [exposed_at],
        )
        self.assertTrue(
            QueryEngine.match(
        ...
        self.assertFalse(
            QueryEngine.match(
                {'items': [{'enrollment_task_id': 'pk_task_soft'}]},
                {'items.exposed_at': {'$exists': True, '$ne': None}},
            )
        )
    ...
    def test_query_engine_supports_top_level_or(self):
        document = {'a': 1, 'b': 3}
        self.assertTrue(
            QueryEngine.match(document, {'$or': [{'a': 2}, {'b': 3}]})
        )
    ...
    def test_query_engine_supports_where_string_expression(self):
        ...
        self.assertTrue(
            QueryEngine.match(
                document, {'$where': "this.age >= 18 and this.role == 'admin'"}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'$where': "this['nested']['score'] > 5"}
            )
        )
        self.assertFalse(
            QueryEngine.match(document, {'$where': 'this.age < 18'})
        )
    ...
    def test_query_engine_supports_where_callable(self):
        ...
        self.assertFalse(
            QueryEngine.match(
                document, {'$where': lambda doc: doc['age'] < 18}
            )
        )

    def test_query_engine_reports_where_runtime_failures(self):
        with self.assertRaisesRegex(
            OperationFailure, '\\$where evaluation failed'
        ):
    ...
    def test_query_engine_supports_top_level_and(self):
        document = {'a': 1, 'b': 3}
        self.assertTrue(
            QueryEngine.match(document, {'$and': [{'a': 1}, {'b': 3}]})
        )
        self.assertFalse(
            QueryEngine.match(document, {'$and': [{'a': 1}, {'b': 4}]})
        )
    ...
    def test_query_engine_mod_handles_large_integers_without_float_precision_loss(
        self,
    ):
        self.assertTrue(
            QueryEngine.match(
    ...
    def test_query_engine_mod_rejects_non_finite_divisors(self):
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'value': 5}, {'value': {'$mod': [float('inf'), 1]}}
            )
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'value': 5}, {'value': {'$mod': [float('nan'), 1]}}
            )
    ...
    def test_query_engine_supports_type_aliases_codes_and_array_semantics(
        self,
    ):
        document = {
            'name': 'Ada',
        ...
        self.assertTrue(
            QueryEngine.match(document, {'name': {'$type': 'string'}})
        )
        self.assertTrue(QueryEngine.match(document, {'score': {'$type': 16}}))
        self.assertTrue(
            QueryEngine.match(document, {'ratio': {'$type': 'number'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'tags': {'$type': 'array'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'tags': {'$type': 'int'}})
        )
        self.assertTrue(QueryEngine.match(document, {'blob': {'$type': 5}}))
        self.assertTrue(
            QueryEngine.match(document, {'oid': {'$type': 'objectId'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'created_at': {'$type': 'date'}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'pattern': {'$type': 'regex'}})
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'missing_like': {'$type': 'undefined'}}
            )
        )
        self.assertFalse(
            QueryEngine.match(document, {'score': {'$type': 'string'}})
        )
        self.assertFalse(
            QueryEngine.match(document, {'missing': {'$type': 'null'}})
        )
    ...
    def test_query_engine_type_rejects_unsupported_or_invalid_type_specs(self):
        ...
        self.assertTrue(
            QueryEngine.match(
                {'score': decimal.Decimal('7.5')},
                {'score': {'$type': 'decimal'}},
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'score': decimal.Decimal('7.5')}, {'score': {'$type': 19}}
            )
        )
        self.assertTrue(
            QueryEngine.match({'score': 7.5}, {'score': {'$type': 'Double'}})
        )
        self.assertTrue(
            QueryEngine.match({'score': 7}, {'score': {'$type': 'INT'}})
        )
        self.assertTrue(
            QueryEngine.match(
                {'score': decimal.Decimal('7.5')},
    ...
    def test_query_engine_supports_bitwise_query_operators(self):
        ...
        self.assertTrue(
            QueryEngine.match(document, {'mask': {'$bitsAllSet': 0b1000}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'mask': {'$bitsAnySet': [0, 1]}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'mask': {'$bitsAllClear': 0b0001}})
        )
        self.assertTrue(
            QueryEngine.match(document, {'mask': {'$bitsAnyClear': [0, 2]}})
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'float_mask': {'$bitsAllSet': 0b0010}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                document, {'binary': {'$bitsAllSet': bytes([0b00000101])}}
            )
        )
        self.assertTrue(
            QueryEngine.match(document, {'negative': {'$bitsAllSet': 1 << 10}})
        )
        self.assertFalse(
            QueryEngine.match(document, {'mask': {'$bitsAllSet': 0b0101}})
        )
        self.assertFalse(
            QueryEngine.match(document, {'array': {'$bitsAnySet': 0b0010}})
        )
    ...
    def test_query_engine_bitwise_rejects_invalid_masks(self):
        ...
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'mask': 0b1010}, {'mask': {'$bitsAnySet': [1, -1]}}
            )
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'mask': 0b1010}, {'mask': {'$bitsAllClear': 1 << 63}}
            )
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'mask': 0b1010}, {'mask': {'$bitsAnyClear': 1.5}}
            )
    ...
    def test_query_engine_supports_all_operator(self):
        ...
        self.assertTrue(
            QueryEngine.match(
                document, {'tags': {'$all': ['python', 'sqlite']}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                document, {'tags': {'$all': ['python', 'redis']}}
            )
        )
        self.assertFalse(QueryEngine.match(document, {'tags': {'$all': []}}))
        self.assertTrue(
            QueryEngine.match(
                {'tags': 'python'}, {'tags': {'$all': ['python']}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                {'tags': 'python'}, {'tags': {'$all': ['python', 'mongodb']}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
    ...
    def test_query_engine_treats_subdocument_order_as_significant_in_in_operator(
        self,
    ):
        self.assertFalse(
            QueryEngine.match(
    ...
    def test_query_engine_treats_subdocument_order_as_significant_in_exact_equality(
        self,
    ):
        self.assertFalse(
            QueryEngine.match(
    ...
    def test_query_engine_supports_size_operator(self):
        ...
        self.assertFalse(
            QueryEngine.match({'tags': 'python'}, {'tags': {'$size': 1}})
        )
        self.assertFalse(
            QueryEngine.match({'tags': [[1, 2]]}, {'tags': {'$size': 2}})
        )

    def test_query_engine_supports_mod_operator(self):
        self.assertTrue(
            QueryEngine.match({'count': 10}, {'count': {'$mod': [3, 1]}})
        )
        self.assertFalse(
            QueryEngine.match({'count': 10}, {'count': {'$mod': [3, 0]}})
        )
        self.assertFalse(
            QueryEngine.match({'count': '10'}, {'count': {'$mod': [3, 1]}})
        )
        self.assertFalse(
            QueryEngine.match(
                {'count': float('inf')}, {'count': {'$mod': [3, 1]}}
            )
        )
        with self.assertRaises(ValueError):
            QueryEngine.match(
                {'count': 10}, {'count': {'$mod': [float('inf'), 1]}}
            )

    def test_query_engine_supports_lte_operator(self):
        self.assertTrue(
            QueryEngine.match({'count': 10}, {'count': {'$lte': 10}})
        )
        self.assertTrue(
            QueryEngine.match({'count': 9}, {'count': {'$lte': 10}})
        )
        self.assertFalse(
            QueryEngine.match({'count': 11}, {'count': {'$lte': 10}})
        )

    def test_query_engine_supports_mod_with_negative_and_float_values(self):
        self.assertFalse(
            QueryEngine.match({'count': -5}, {'count': {'$mod': [3, 1]}})
        )
        self.assertTrue(
            QueryEngine.match({'count': -5}, {'count': {'$mod': [3, -2]}})
        )
        self.assertTrue(
            QueryEngine.match({'count': 5.5}, {'count': {'$mod': [2.5, 0.5]}})
        )
        self.assertFalse(
            QueryEngine.match({'count': 5.5}, {'count': {'$mod': [2.5, 0.0]}})
        )

    def test_query_engine_supports_regex_operator(self):
        self.assertTrue(
            QueryEngine.match({'name': 'Ada'}, {'name': {'$regex': '^Ad'}})
        )
        self.assertTrue(
            QueryEngine.match(
                {'name': 'ada'}, {'name': {'$regex': '^ad', '$options': 'i'}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'tags': ['python', 'mongodb']}, {'tags': {'$regex': '^py'}}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'name': 'ada'},
                {'name': {'$regex': re.compile('^ad', re.IGNORECASE)}},
            )
        )
        self.assertFalse(
            QueryEngine.match({'name': 'Grace'}, {'name': {'$regex': '^Ad'}})
        )

    def test_query_engine_supports_implicit_and_explicit_eq_regex_literals(
        self,
    ):
        self.assertTrue(
            QueryEngine.match(
                {'name': 'MongoDB2'}, {'name': re.compile('MongoDB')}
            )
        )
        self.assertTrue(
            QueryEngine.match(
                {'name': 'MongoDB2'}, {'name': {'$eq': re.compile('MongoDB')}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
    ...
    def test_query_engine_supports_not_operator(self):
        self.assertTrue(
            QueryEngine.match(
                {'name': 'Ada'}, {'name': {'$not': {'$regex': '^Gr'}}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                {'name': 'Ada'}, {'name': {'$not': {'$regex': '^Ad'}}}
            )
        )
        self.assertTrue(
            QueryEngine.match({'value': 2}, {'value': {'$not': {'$gt': 3}}})
        )
    ...
    def test_query_engine_supports_not_with_elem_match(self):
        ...
        self.assertTrue(
            QueryEngine.match(
                document, {'scores': {'$not': {'$elemMatch': {'$gt': 7}}}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                document,
                {'scores': {'$not': {'$elemMatch': {'$gt': 3, '$lt': 5}}}},
            )
        )
    ...
    def test_query_engine_supports_elem_match_for_scalars_and_documents(self):
        ...
        self.assertFalse(
            QueryEngine.match(
                {'items': 'x'}, {'items': {'$elemMatch': {'kind': 'b'}}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                {'items': [1, 2]}, {'items': {'$elemMatch': {'kind': 'b'}}}
            )
        )

    def test_query_engine_supports_elem_match_with_compound_subdocument_operators(
        self,
    ):
        document = {
            'items': [
    ...
    def test_query_engine_elem_match_reuses_compiled_plans(self):
        scalar_plan = compile_filter(
            {'scores': {'$elemMatch': {'$gt': 3, '$lt': 5}}}
        )
        document_plan = compile_filter(
            {'items': {'$elemMatch': {'kind': 'b', 'qty': 2}}}
        )
        ...
        with patch.object(
            QueryEngine,
            'match',
            side_effect=AssertionError('unexpected runtime recompilation'),
        ):
            self.assertTrue(
                QueryEngine.match_plan(
                    {'scores': [1, 4, 7]},
    ...
    def test_query_engine_elem_match_falls_back_when_runtime_dialect_differs(
        self,
    ):
        document = {'items': [UNDEFINED]}
        plan_70 = compile_filter(
            {'items': {'$elemMatch': {'$eq': None}}},
            dialect=MONGODB_DIALECT_70,
        )
        plan_80 = compile_filter(
            {'items': {'$elemMatch': {'$eq': None}}},
    ...
    def test_query_engine_reuses_compiled_json_schema_validator(self):
        ...
        with patch(
            'mongoeco.core.schema_validation.CompiledJsonSchema',
            side_effect=AssertionError('unexpected recompilation'),
        ):
            self.assertTrue(QueryEngine.match_plan({'name': 'Ada'}, plan))

    def test_query_engine_elem_match_private_helper_supports_direct_scalar_condition(
        self,
    ):
        self.assertTrue(
            QueryEngine._match_elem_match_candidate('python', 'python')
        )
        self.assertFalse(
            QueryEngine._match_elem_match_candidate('python', 'mongodb')
        )

    def test_query_engine_supports_eq_with_regex_literals(self):
        self.assertTrue(
            QueryEngine.match(
                {'name': 'Ada'}, {'name': {'$eq': re.compile('^Ad')}}
            )
        )
        self.assertFalse(
            QueryEngine.match(
                {'name': 'Grace'}, {'name': {'$eq': re.compile('^Ad')}}
            )
        )
    ...
    def test_query_engine_rejects_unsupported_regex_option(self):
        with self.assertRaises(OperationFailure):
            QueryEngine.match(
                {'name': 'Ada'}, {'name': {'$regex': '^Ad', '$options': 'z'}}
            )
