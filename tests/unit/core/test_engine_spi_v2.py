...
class EngineSpiV2ContractTests(unittest.TestCase):
    def test_adapter_argument_resolution_has_explicit_missing_semantics(self):
        ...
        assert adapter_module._call_argument(
            (),
            {},
            name='value',
            index=2,
    ...
    def test_v2_adapter_rejects_invalid_batch_and_snapshot_contracts(self):
        ...
        engine = _NativeEngine()
        engine.open_read_snapshot = lambda *_args, **_kwargs: ReadSnapshot(
            _EmptySource(),
            policy=SnapshotPolicy.STABLE,
    ...
    def test_v2_adapter_falls_back_without_a_native_batch(self):
        ...
        async def _exercise():
            ...
            insert_calls = [
                call for call in engine.calls if call[0] == 'insert'
            ]
            ...
            assert [
                call[2]['operation_context'].change_event_index
                for call in insert_calls
            ] == [0, 1]
            assert all(
                call[2]['operation_context'].operation_id
