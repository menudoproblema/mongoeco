...
def _metadata_document(path: str, value: object) -> dict[str, object]:
    document: dict[str, object] = {}
    set_document_value(document, path, value)
    return document


class AggregationRuntimeTests(unittest.TestCase):
    def test_aggregation_runtime_helpers_cover_runtime_key_bson_and_missing_expression_paths(
        self,
    ):
        self.assertEqual(_aggregation_key(1 << 40), ("long", 1 << 40))
        self.assertEqual(_aggregation_key(1.5), ("float", 1.5))
        self.assertEqual(
            _aggregation_key(decimal.Decimal("1.5")),
            ("decimal", decimal.Decimal("1.5")),
        )
        self.assertEqual(
            _aggregation_key(Timestamp(1, 2)),
        ...
        self.assertEqual(
            evaluate_expression(
                _metadata_document(TEXT_SCORE_FIELD, 1.5),
                {"$meta": "textScore"},
            ),
            1.5,
        )
        self.assertEqual(
            evaluate_expression(
                _metadata_document(VECTOR_SEARCH_SCORE_FIELD, 2.5),
                {"$meta": "vectorSearchScore"},
            ),
            2.5,
        )
        highlights = [{"path": "title", "text": "Ada"}]
        self.assertEqual(
            evaluate_expression(
                _metadata_document(SEARCH_HIGHLIGHTS_METADATA_FIELD, highlights),
        ...
        self.assertEqual(
            evaluate_expression(
                {"name": "Ada"},
                {"present": "$name", "missing": "$unknown"},
            ),
            {"present": "Ada", "missing": None},
        )
        self.assertIs(
            _evaluate_expression_with_missing(
                {"items": []},
        ...
        self.assertEqual(
            evaluate_expression({}, {"kept": 1, "dropped": _MISSING}),
            {"kept": 1},
        )
        with self.assertRaisesRegex(OperationFailure, "only supports"):
    ...
    def test_transform_stage_helpers_cover_missing_add_fields_and_replace_root_validation(
        self,
    ):
        self.assertEqual(
            apply_pipeline(
        ...
        with self.assertRaisesRegex(
            OperationFailure,
            "newRoot must evaluate to a document",
        ):
            apply_pipeline(
                [{"_id": 1, "name": "Ada"}],
