...
class UpdateEngineTests(unittest.TestCase):
    ...
    def test_update_rejects_other_unsupported_update_operators_explicitly(
        self,
    ):
        unsupported_specs = [
            {'$pushAll': {'tags': ['python']}},
    ...
    def test_compile_update_pipeline_plan_rejects_array_filters_for_pipeline_updates(
        self,
    ):
        context = UpdateEngine.build_execution_context(
            array_filters=[{'item.qty': {'$gt': 1}}]
    ...
    def test_array_filter_matches_handles_missing_identifier_and_non_document_candidates(
        self,
    ):
        self.assertFalse(
    ...
    def test_legacy_positional_matcher_covers_and_recursion_and_elem_match(
        self,
    ):
    ...
    def test_array_filters_validate_identifiers_and_reject_legacy_positional_paths(
        self,
    ):
        with self.assertRaises(OperationFailure):
            UpdateEngine.apply_update(
    ...
    def test_add_to_set_with_each_deduplicates_when_creating_missing_array(
        self,
    ):
        document = {}
