...
class AsyncAggregationCursorTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_meta_helper_supports_facet_operator_collector_runtime_projection(
        self,
    ):
        collection = _FakeCollection([])
    ...
    def test_search_runtime_spec_projection_keeps_ambiguous_facet_operator_unchanged(
        self,
    ):
        original_spec = {
            'index': 'by_text',
    ...
    def test_build_pushdown_cursor_reraises_unrelated_type_errors_from_builder(
        self,
    ):
    ...
    async def test_leading_search_downstream_filter_keeps_or_match_with_sibling_fields(
        ...
        for engine_cls in (MemoryEngine, SQLiteEngine):
            with self.subTest(engine=engine_cls.__name__):
                async with AsyncMongoClient(engine_cls()) as client:
    ...
    async def test_collection_aggregate_accepts_pymongo_allow_disk_use_alias(
        self,
    ):
    ...
    async def test_streaming_batch_execution_skips_full_first_page_and_honors_zero_remaining_limit(
        self,
    ):
        collection = _FakeCollection(
            [
    ...
    async def test_materialize_rejects_large_blocking_pipeline_without_spill_budget(
        self,
    ):
        collection = _FakeCollection(
    ...
    async def test_stream_batches_consumes_entire_batch_when_trailing_skip_exceeds_transformed_size(
        ...
        with (
            ...
            cursor._build_pushdown_cursor = lambda _operation: _BatchCursor(
                batches.pop(0)
            )  # type: ignore[method-assign]
            self.assertEqual(
                [document async for document in cursor._stream_batches()], []
