...
class AsyncCollectionManagementTests(AsyncCollectionHelperBase):
    ...
    def test_create_index_rejects_invalid_expire_after_seconds_type(self):
        ...
        with self.assertRaises(TypeError):
            asyncio.run(
                collection.create_index([("expires_at", 1)], expire_after_seconds=True)
            )
    ...
    def test_search_index_helpers_rename_watch_and_options_delegate_correctly(
        ...
        engine, created, created_many, listed, renamed, options, stream = asyncio.run(
            _exercise()
        )
    ...
    def test_update_one_hint_upserts_when_no_document_matches(self):
        ...
        async def _exercise():
            ...
            collection._perform_upsert_update = lambda *args, **kwargs: asyncio.sleep(
                0,
                result=UpdateResult(matched_count=0, modified_count=0, upserted_id="1"),
    ...
    def test_find_one_and_update_handles_upsert_after_lookup_and_missing_after_document(
        ...
        async def _exercise_upsert():
            collection = AsyncCollection(MemoryEngine(), "db", "coll")
            collection._select_first_document = lambda *args, **kwargs: asyncio.sleep(
                0, result=None
            )  # type: ignore[method-assign]
    ...
    def test_delete_one_hint_returns_zero_when_selection_is_empty(self):
        ...
        async def _exercise():
            collection = AsyncCollection(MemoryEngine(), "db", "coll")
            collection._build_cursor = lambda *args, **kwargs: CursorStub()  # type: ignore[method-assign]
            return await collection.delete_one({"_id": "missing"}, hint="_id_")
