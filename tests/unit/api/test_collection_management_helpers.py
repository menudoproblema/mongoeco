...
class AsyncCollectionManagementTests(AsyncCollectionHelperBase):
    def test_find_and_aggregate_match_initial_or_filter_with_sibling_fields(
        self,
    ):
        async def _exercise():
            engine = MemoryEngine()
            ...
            try:
                collection = AsyncCollection(engine, 'db', 'tasks')
                expected = {
                    '_id': 'task-1',
                ...
                found = await collection.find(filter_spec).to_list()
                aggregated = await collection.aggregate(
                    [{'$match': filter_spec}]
                ).to_list()
                return expected, found, aggregated
            finally:
                await engine.disconnect()

        expected, found, aggregated = asyncio.run(_exercise())

        self.assertEqual(found, [expected])
        self.assertEqual(aggregated, [expected])

    def test_aggregate_pushdown_streaming_merges_or_match_with_sibling_match_filters(
        self,
    ):
        async def _exercise(engine_cls):
            engine = engine_cls()
            await engine.connect()
            try:
                collection = AsyncCollection(engine, 'db', 'tasks')
        ...
        for engine_cls in (MemoryEngine, SQLiteEngine):
            with self.subTest(engine=engine_cls.__name__):
                documents = asyncio.run(_exercise(engine_cls))

                self.assertEqual(documents, [{'_id': 'task-1'}])
    ...
    def test_find_one_returns_projection_already_applied_by_engine(self):
        ...
        direct = asyncio.run(
            collection.find_one({'_id': '1'}, {'name': 1, '_id': 0})
        )
        scanned = asyncio.run(
            collection.find_one({'name': 'Ada'}, {'role': 0, '_id': 1})
        )
    ...
    def test_collection_compiles_plan_once_and_passes_it_to_engine(self):
        class EngineStub:
            ...
            def scan_find_semantics(
                self, db_name, coll_name, semantics, *, context=None
            ):
        ...
        asyncio.run(
            collection.update_one({'name': 'Ada'}, {'$set': {'active': True}})
        )
        asyncio.run(collection.delete_one({'name': 'Ada'}))
        asyncio.run(collection.count_documents({'name': 'Ada'}))

        self.assertEqual(
            type(engine.scan_semantics_history[0].query_plan).__name__,
            'EqualsCondition',
        )
        ...
        self.assertNotIsInstance(
            engine.scan_semantics_history[0].query_plan, MatchAll
        )
        self.assertNotIsInstance(engine.count_plan, MatchAll)
        self.assertIs(
            engine.scan_semantics_history[0].dialect,
            collection.mongodb_dialect,
        )
    ...
    def test_list_indexes_returns_cursor(self):
        ...
        self.assertEqual(
            first, {'name': '_id_', 'key': {'_id': 1}, 'unique': True}
        )
        self.assertEqual(
            indexes,
            [
    ...
    def test_create_indexes_requires_index_models(self):
        ...
        with self.assertRaises(TypeError):
            asyncio.run(
                collection.create_indexes((IndexModel([('email', 1)]),))
    ...
    def test_insert_many_accepts_iterable_but_rejects_mappings_and_empty_iterables(
        self,
    ):
        collection = AsyncCollection(MemoryEngine(), 'db', 'coll')
        ...
        async def _insert_generator():
            return await collection.insert_many(
                ({'name': name} for name in ('Ada', 'Grace'))
            )
    ...
    def test_create_index_accepts_ordered_mapping_key_spec(self):
        ...
        name = asyncio.run(
            collection.create_index(
                {'email': 1, 'created_at': -1}, unique=True
            )
        )
    ...
    def test_create_index_forwards_collation(self):
        ...
        asyncio.run(
            collection.create_index([('email', 1)], collation={'locale': 'en'})
        )
    ...
    def test_create_index_rejects_invalid_expire_after_seconds_type(self):
        ...
        with self.assertRaises(TypeError):
            asyncio.run(
                collection.create_index(
                    [('expires_at', 1)], expire_after_seconds=-1
                )
            )
        ...
        with self.assertRaises(TypeError):
            asyncio.run(
                collection.create_index(
                    [('expires_at', 1)], expire_after_seconds=True
                )
            )
    ...
    def test_create_indexes_shares_batch_deadline_across_calls(self):
        ...
        self.assertTrue(
            all(isinstance(value, int) and value > 0 for value in engine.calls)
        )
    ...
    def test_update_one_sort_updates_first_sorted_match_when_profile_supports_it(
        self,
    ):
        async def _exercise() -> tuple[object, dict | None, dict | None]:
            ...
            await collection.insert_one(
                {'_id': 'first', 'kind': 'view', 'rank': 2, 'done': False}
            )
            await collection.insert_one(
                {'_id': 'second', 'kind': 'view', 'rank': 1, 'done': False}
            )
    ...
    def test_update_one_sort_returns_zero_counts_when_no_document_matches(
        self,
    ):
        async def _exercise():
            collection = AsyncCollection(
    ...
    def test_update_one_sort_uses_custom_dialect_for_document_selection(self):
        ...
        async def _exercise(engine):
            ...
            try:
                await collection.insert_one(
                    {'_id': 'low', 'kind': 'view', 'rank': 1, 'done': False}
                )
                await collection.insert_one(
                    {'_id': 'high', 'kind': 'view', 'rank': 2, 'done': False}
                )
    ...
    def test_custom_projection_dialect_is_honored_by_find_and_find_one(self):
        ...
        async def _exercise(engine):
            ...
            try:
                ...
                direct = await collection.find_one(
                    {'_id': '1'}, {'name': 2, '_id': 0}
                )
                scanned = await collection.find(
                    {'name': 'Ada'}, {'name': 2, '_id': 0}
                ).to_list()
    ...
    def test_engine_update_and_delete_with_operation_profile_errors(self):
        ...
        async def _exercise():
            ...
            with self.assertRaisesRegex(RuntimeError, 'delete boom'):
                await collection._engine_delete_with_operation(
                    delete_operation
                )
            return profiled
        ...
        self.assertEqual(
            [entry['op'] for entry in profiled], ['update', 'remove']
        )
    ...
    def test_search_index_helpers_rename_watch_and_options_delegate_correctly(
        ...
        async def _exercise():
            ...
            created = await collection.create_search_index(
                {'mappings': {'dynamic': True}}, max_time_ms=25
            )
            ...
            await collection.update_search_index(
                'search_idx', {'mappings': {'dynamic': False}}, max_time_ms=10
            )
        ...
        engine, created, created_many, listed, renamed, options, stream = (
    ...
    def test_collection_properties_and_with_options_expose_normalized_configuration(
        self,
    ):
        collection = self.collection.with_options(
            planning_mode=Enum('Mode', {'RELAXED': 'relaxed'}).RELAXED
        )
    ...
    def test_apply_codec_options_to_document_rejects_non_dict_mapping_outputs(
        self,
    ):
        collection = AsyncCollection(MemoryEngine(), 'db', 'coll')

        with patch.object(
            async_collection_module.DocumentCodec,
            'apply_codec_options',
            return_value=[],
        ):
    ...
    def test_rename_preserves_collection_configuration(self):
        ...
        self.assertEqual(
            renamed.change_stream_history_size,
            collection.change_stream_history_size,
        )
        self.assertEqual(
            renamed.change_stream_journal_path,
            collection.change_stream_journal_path,
        )
        self.assertEqual(
            renamed.change_stream_journal_fsync,
            collection.change_stream_journal_fsync,
        )
        self.assertEqual(
            renamed.change_stream_journal_max_bytes,
            collection.change_stream_journal_max_bytes,
        )
    ...
    def test_database_preserves_change_stream_journal_configuration(self):
        ...
        self.assertEqual(
            database.change_stream_journal_path, '/tmp/mongoeco-journal.json'
        )
        self.assertTrue(database.change_stream_journal_fsync)
        self.assertEqual(database.change_stream_journal_max_bytes, 2048)

    def test_bulk_write_context_prepare_request_covers_variants_and_errors(
        self,
    ):
        collection = AsyncCollection(
            MemoryEngine(), 'db', 'coll', pymongo_profile='4.11'
        )
        context = async_collection_module._BulkWriteContext(collection, [])

        prepared_insert = context._prepare_request(
            0, InsertOne({'name': 'Ada'})
        )
        ...
        prepared_update_error = context._prepare_request(
            2,
            UpdateOne(
                {'_id': '1'}, {'$set': {'done': True}}, array_filters='bad'
            ),  # type: ignore[arg-type]
        )
    ...
    def test_create_indexes_rolls_back_created_indexes_when_later_creation_fails(
        ...
        async def _exercise():
            ...
            await collection.create_indexes(
                [
                    IndexModel([('a', 1)], name='idx_a'),
                    IndexModel([('b', 1)], name='idx_b'),
                ]
            )
    ...
    def test_update_one_hint_and_change_hub_paths_publish_expected_events(
        self,
    ):
        class HubStub:
            def __init__(self):
        ...
        self.assertEqual(
            [event['operation_type'] for event in events], ['update', 'update']
        )
    ...
    def test_update_one_hint_upserts_when_no_document_matches(self):
        ...
        async def _exercise():
            ...
            collection._perform_upsert_update = lambda *args, **kwargs: (
                asyncio.sleep(
                    0,
                    result=UpdateResult(
    ...
    def test_find_one_and_update_handles_upsert_after_lookup_and_missing_after_document(
        self,
    ):
        class CursorStub:
            def __init__(self, document):
        ...
        async def _exercise_upsert():
            ...
            collection.update_one = lambda *args, **kwargs: asyncio.sleep(
                0,
                result=UpdateResult(
                    matched_count=0, modified_count=0, upserted_id='1'
                ),
            )  # type: ignore[method-assign]
            collection.find = lambda *args, **kwargs: CursorStub(
                {'_id': '1', 'done': True}
            )  # type: ignore[method-assign]
        ...
        self.assertEqual(
            asyncio.run(_exercise_upsert()), {'_id': '1', 'done': True}
        )
        self.assertIsNone(asyncio.run(_exercise_missing_after()))

    def test_find_one_and_replace_returns_none_when_after_document_disappears(
        self,
    ):
        async def _exercise():
            collection = AsyncCollection(MemoryEngine(), 'db', 'coll')
            ...
            collection.replace_one = lambda *args, **kwargs: asyncio.sleep(
                0, result=UpdateResult(matched_count=1, modified_count=1)
            )  # type: ignore[method-assign]
            collection._document_by_id = lambda *args, **kwargs: asyncio.sleep(
                0, result=lookup_results.pop(0)
    ...
    def test_delete_one_change_hub_and_hint_paths_cover_selection_branches(
        self,
    ):
        class HubStub:
            def __init__(self):
