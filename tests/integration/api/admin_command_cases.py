...
async def assert_client_server_info_reflects_target_dialect(
    case, open_client: OpenClient,
) -> None:
...
async def assert_build_info_command_shares_source_of_truth_with_server_info(
    case, open_client: OpenClient,
) -> None:
    async with open_client("memory", mongodb_dialect="8.0") as client:
        server_info = await _maybe_await(client.server_info())
...
async def assert_hello_and_is_master_commands_return_handshake_metadata(
    case, open_client: OpenClient,
) -> None:
    async with open_client("memory", mongodb_dialect="8.0") as client:
...
async def assert_server_status_command_returns_local_runtime_metadata(
    case, engine_names, open_client: OpenClient,
) -> None:
        ...
        with case.subTest(engine=engine_name):
            async with open_client(engine_name, mongodb_dialect="8.0") as client:
                ...
                case.assertGreaterEqual(
                    status["mongoeco"]["adminCommandSurfaceCount"], 1,
                )
                case.assertGreaterEqual(
                    status["mongoeco"]["wireCommandSurfaceCount"], 1,
                )
                case.assertGreaterEqual(
                    status["mongoeco"]["adminFamilies"]["admin_read"], 1,
                )
                case.assertGreaterEqual(
                    status["mongoeco"]["explainableCommandCount"], 1,
                )
                ...
                case.assertEqual(
                    status["mongoeco"]["changeStreams"]["implementation"],
                    "local",
                )
                ...
                case.assertIn(
                    "hybridSortFallback",
                    status["mongoeco"]["engineRuntime"]["planner"],
                )
                case.assertIn(
                    "simulatedIndexLatencySeconds",
                ...
                if engine_name == "memory":
                    case.assertEqual(
                        status["mongoeco"]["engineRuntime"]["planner"]["engine"],
                        "python",
                    )
                    case.assertEqual(
...
async def assert_server_status_opcounters_track_local_runtime_activity(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name, mongodb_dialect="8.0") as client:
                ...
                cursor = await _maybe_await(
                    collection.find({}, sort=[("seq", 1)], batch_size=1),
                )
                ...
                aggregation_cursor = await _maybe_await(
                    collection.aggregate([{"$match": {"kind": "view"}}]),
                )
...
async def assert_database_command_supports_db_hash_and_profile_status(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                filtered_hash = await _maybe_await(
                    database.command({"dbHash": 1, "collections": ["users"]}),
                )
                profile_enabled = await _maybe_await(
                ...
                case.assertEqual(
                    filtered_hash["collections"],
                    {"users": full_hash["collections"]["users"]},
                )
...
async def assert_host_info_whats_my_uri_and_cmd_line_opts_commands_return_local_metadata(
    case, open_client: OpenClient,
) -> None:
    async with open_client("memory", mongodb_dialect="8.0") as client:
...
async def assert_list_collections_command_supports_name_only(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
    ...
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                case.assertEqual(
                    await _maybe_await(client.alpha.command("ping")),
                    {"ok": 1.0},
                )
                case.assertEqual(
                    await _maybe_await(
                        client.alpha.command(
                            "listCollections", filter={"name": "events"},
                        ),
                    ),
                    {
                        "cursor": {
                ...
                case.assertNotIn(
                    "alpha", await _maybe_await(client.list_database_names()),
                )


async def assert_database_command_supports_collection_index_count_and_distinct_commands(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                case.assertEqual(
                    await _maybe_await(
                        client.alpha.command(
                            {"create": "events", "capped": True, "size": 512},
                        ),
                    ),
                    {"ok": 1.0},
                )
                ...
                case.assertEqual(
                    await _maybe_await(
                        client.alpha.command(
                            {"dropIndexes": "events", "index": "kind_idx"},
                        ),
                    ),
...
async def assert_database_command_index_commands_support_comment_and_max_time(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_count_supports_skip_limit_hint_and_comment(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_distinct_supports_hint_comment_and_max_time(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_supports_rename_collection_within_current_database(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                case.assertEqual(
                    await _maybe_await(
                        client.alpha.protected.find_one({"_id": "keep"}),
                    ),
                    {"_id": "keep"},
                )
                ...
                await _maybe_await(
                    client.alpha.events.insert_one(
                        {"_id": "ttl-1", "expires_at": "soon"},
                    ),
                )
                await _maybe_await(
                    client.alpha.events.create_index([("kind", 1)], name="kind_idx"),
                ...
                case.assertIn(
                    "kind_idx",
                    await _maybe_await(client.alpha.archived.index_information()),
                )


async def assert_database_command_supports_find_and_aggregate(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_supports_explain_for_find_and_aggregate(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_supports_explain_for_update_and_delete(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_supports_explain_for_count_distinct_and_find_and_modify(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                case.assertEqual(
                    find_and_modify_explain["explained_command"],
                    "findAndModify",
                )
...
async def assert_database_command_supports_insert_update_and_delete(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_write_commands_surface_write_errors(
    case, open_client: OpenClient,
) -> None:
...
async def assert_database_command_supports_find_and_modify(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
...
async def assert_database_command_supports_coll_stats_and_db_stats(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                coll_stats = await _maybe_await(
                    client.alpha.command({"collStats": "events"}),
                )
                db_stats = await _maybe_await(client.alpha.command("dbStats"))
                scaled_coll_stats = await _maybe_await(
                    client.alpha.command({"collStats": "events", "scale": 2}),
                )
                scaled_db_stats = await _maybe_await(
                    client.alpha.command({"dbStats": 1, "scale": 2}),
                )
                ...
                case.assertLessEqual(
                    scaled_coll_stats["storageSize"], coll_stats["storageSize"],
                )
                case.assertLessEqual(
                    scaled_coll_stats["totalIndexSize"],
                    coll_stats["totalIndexSize"],
                )
                case.assertEqual(scaled_coll_stats["scaleFactor"], 2)
                case.assertLessEqual(scaled_db_stats["dataSize"], db_stats["dataSize"])
                case.assertLessEqual(
                    scaled_db_stats["storageSize"], db_stats["storageSize"],
                )
                case.assertLessEqual(
                    scaled_db_stats["indexSize"], db_stats["indexSize"],
                )
                case.assertEqual(scaled_db_stats["scaleFactor"], 2)


async def assert_database_command_supports_configure_fail_point(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                    ...
                    client.alpha.command(
                        {
                            ...
                            "data": {
                                "failCommands": ["count"],
                                "namespaces": ["events", "orders"],
                            },
                        },
                    ),
                )
                case.assertEqual(
                    multi_ns["data"]["namespaces"],
                    ["alpha.events", "alpha.orders"],
                )
                with case.assertRaisesRegex(OperationFailure, "failpoint"):
                    await _maybe_await(client.alpha.command({"count": "events"}))
                with case.assertRaisesRegex(OperationFailure, "failpoint"):
                    await _maybe_await(client.alpha.command({"count": "orders"}))
                case.assertEqual(
                    (
                        await _maybe_await(
                            client.alpha.command({"count": "other_events"}),
                ...
                with case.assertRaisesRegex(
                    OperationFailure, "simulated write concern timeout",
                ) as wtimeout_ctx:
                    await _maybe_await(client.alpha.command({"count": "events"}))
                write_concern_error = wtimeout_ctx.exception.details.get(
                    "writeConcernError", {},
                )
                ...
                write_command_error = write_wtimeout_ctx.exception.details.get(
                    "writeConcernError", {},
                )
...
async def assert_database_command_rejects_unsupported_commands(
    case, open_client: OpenClient,
) -> None:
    async with open_client("memory") as client:
...
async def assert_database_command_rejects_invalid_command_shapes(
    case, open_client: OpenClient,
) -> None:
    async with open_client("memory") as client:
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"createIndexes": "events", "indexes": ()}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"distinct": "events", "key": 1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command("listDatabases", nameOnly=1),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command("listCollections", nameOnly=1),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command("listCollections", authorizedCollections=1),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"count": "events", "limit": -1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"collStats": "events", "scale": -1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"insert": "events", "documents": {}}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"update": "events", "updates": {}}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"delete": "events", "deletes": {}}),
            )  # type: ignore[arg-type]
        with case.assertRaises(ValueError):
            await _maybe_await(
                client.alpha.command({"find": "events", "batchSize": -1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command(
                    {"aggregate": "events", "pipeline": [], "cursor": 1},
                ),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command(
                    {"aggregate": "events", "pipeline": [], "allowDiskUse": 1},
                ),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"aggregate": "events", "pipeline": [], "let": 1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"find": "events", "sort": 1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"renameCollection": "events", "to": "alpha.logs"}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(client.alpha.command({"distinct": "events", "key": ""}))
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command("listCollections", comment=1),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"listIndexes": "events", "comment": 1}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"validate": "events", "background": "yes"}),
            )  # type: ignore[arg-type]
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"count": "events", "limit": -1}),
        ...
        with case.assertRaises(TypeError):
            await _maybe_await(
                client.alpha.command({"explain": {"distinct": "events", "key": ""}}),
            )
        with case.assertRaises(TypeError):
            await _maybe_await(
        ...
        with case.assertRaises(OperationFailure):
            await _maybe_await(
                client.alpha.command(
                    {"renameCollection": "alpha.events", "to": "beta.logs"},
                ),
            )


async def assert_validate_collection_returns_metadata_and_rejects_missing_namespace(
    case, engine_names, open_client: OpenClient,
) -> None:
    for engine_name in engine_names:
        with case.subTest(engine=engine_name):
            async with open_client(engine_name) as client:
                ...
                validated = await _maybe_await(
                    client.alpha.validate_collection("events"),
                )
                validated_from_command = await _maybe_await(
                    client.alpha.command({"validate": "events"}),
                )
                ...
                case.assertEqual(
                    validated["keysPerIndex"],
                    {"_id_": 1, "kind_idx": 1, "expires_at_ttl": 1},
                )
